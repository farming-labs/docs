...
All cross-framework built-in themes, including `concrete` and `shadcn`, are available across Next.js, TanStack Start, SvelteKit, Astro, and Nuxt.
...
The Shadcn Docs preset follows the same package pattern: import `shadcn` from the framework's `/shadcn` entrypoint and pair it with that package's `/shadcn/css` stylesheet.
