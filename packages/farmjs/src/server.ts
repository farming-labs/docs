...
import { createFarmjsApiReference } from "./api-reference.js";
...
export function createDocsServer(config: Record<string, any>): DocsServer {
  ...
  async function handle(request: Request): Promise<Response | null> {
    ...
    if (mcpConfig.enabled && mcpRoutes.has(pathname)) {
      const mcpHandler = MCP[method as keyof DocsMcpHttpHandlers];
      if (mcpHandler) return mcpHandler({ request });
    ...
    if (apiReference.enabled && pathname === `/${apiReference.path}`) {
      ...
      return createFarmjsApiReference(config as any)();
    }

    const isApiRoute = pathname === "/api/docs" || pathname.startsWith("/api/docs/");
    const isPublicGetRoute =
      isDocsConfigRequest(url) ||
      isDocsDiagnosticsRequest(url) ||
      isDocsAgentDiscoveryRequest(url) ||
      isDocsAgentsRequest(url) ||
      isDocsSkillRequest(url) ||
      Boolean(resolveDocsAgentFeedbackRequest(url, agentFeedbackConfig)) ||
      Boolean(resolveDocsMarkdownRequest(entry, url, request)) ||
      Boolean(resolveDocsLlmsTxtRequest(url, llmsTxtConfig, entry)) ||
      Boolean(resolveDocsSitemapRequest(url, config.sitemap)) ||
      Boolean(resolveDocsRobotsRequest(url, config.robots));

    if (method === "GET" || method === "HEAD") {
      return isApiRoute || isPublicGetRoute ? GET({ request }) : null;
    }
    ...
    if (isApiRoute || isPublicGetRoute) {
      return new Response("Method Not Allowed", {
        status: 405,
  ...
  return { load, GET, POST, MCP, handle };
}
