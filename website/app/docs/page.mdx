...
- **CLI scaffolding** — `npx @farming-labs/docs init` first asks whether you're on an **existing project** or **starting fresh**; then it detects your framework, lets you pick a theme (or create your own), and scaffolds config, routes, and sample pages. Use `--template` with `--name` to bootstrap a new project without prompts.
