...
agent:
  ...
  outcome: The project has a working docs entry plus discoverable Markdown, search, sitemap, robots, and MCP capabilities appropriate to its config.
  appliesTo:
  ...
  commands:
    - run: pnpm dlx @farming-labs/docs init
      description: Start the existing-project setup flow from the application root.
  sideEffects:
    - Initialization installs packages and scaffolds framework-specific docs files.
  verification:
    - description: Open the configured docs entry and fetch /.well-known/agent.json from the same origin.
      expect: The docs page renders and discovery advertises only the capabilities enabled by the resolved config.
  rollback:
    - Restore dependency and lock files and remove only the scaffolded docs files created by initialization.
  failureModes:
    - symptom: The initializer selects the wrong framework.
      resolution: Stop before accepting generated changes and rerun with the documented explicit template or framework choice.
    - symptom: An agent starts scraping HTML for implementation details.
      resolution: Read /.well-known/agent.json first and use its Markdown, search, schema, sitemap, or MCP routes.
---
...
<Callout type="tip" title="Docs Review CI is built in">
Every docs site gets PR review automation by default. During `next dev` or `next build`, `withDocs()`
can create a repo-root GitHub Actions wrapper that calls the hosted review workflow, checks changed
docs files, scores agent-readiness, and reports GitHub annotations. Keep it as warn-only, or set
`review.ci.mode: "block"` when docs quality should gate merges. See [Docs Review CI](/docs/configuration#docs-review-ci) and the [`docs review` CLI](/docs/cli#docs-review).
</Callout>
...
key and sync `docs.json`. Use [Deploy](/docs/cloud/deploy) for hosted preview docs and
[Analytics](/docs/cloud/analytics) for Cloud-managed event storage.
...
- **[Docs Review CI](/docs/configuration#docs-review-ci)** — Generated GitHub Actions wrapper plus hosted `docs review` workflow scoring for changed MDX, broken internal links, frontmatter, code fence metadata, and agent context.
...
- [Docs Cloud](/docs/cloud) — Understand the Cloud project contract
- [Deploy](/docs/cloud/deploy) — Connect a docs project to hosted previews, API keys, and publish defaults
- [Analytics](/docs/cloud/analytics) — Understand Cloud-managed analytics storage
