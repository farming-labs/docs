...
can create a repo-root GitHub Actions workflow that reviews changed docs files, checks frontmatter
and internal links, scores agent-readiness, and reports GitHub annotations. Keep it as warn-only, or
set `review.ci.mode: "block"` when docs quality should gate merges. See [Docs Review CI](/docs/configuration#docs-review-ci) and the [`docs review` CLI](/docs/cli#docs-review).
</Callout>
...
- **[Docs Review CI](/docs/configuration#docs-review-ci)** — Generated GitHub Actions workflow plus `docs review` scoring for changed MDX, broken internal links, frontmatter, code fence metadata, and agent context.
