...
- agents can discover valid filter values (framework, version, package, tags) without a text query
  using `GET /api/docs?audience=agent&response=facets` (returns `DocsSearchFacetsResponse` with
  `format: "docs-search-facets.v1"`, cached `max-age=60`); the agent discovery spec advertises this
  as `search.facetsEndpoint`, `search.facetsResponseValue: "facets"`, and
...
[Agent Skills v0.2 index](https://www.mintlify.com/docs/ai/skillmd). Configure `agent.skills` with a
project-relative `SKILL.md`, skill directory, collection directory, or array of those paths. The
runtime discovers every valid skill and publishes only non-symlink regular files from `SKILL.md`,
`references/`, `scripts/`, and `assets/`.

Standalone skill documents are indexed directly. Skills with companions use a deterministic
`.tar.gz` artifact whose SHA-256 digest covers the exact archive bytes, while each source file stays
available below `/.well-known/agent-skills/<name>/`. Legacy clients can use
`/.well-known/skills/index.json`; static exports reproduce both indexes and their artifacts. MCP
clients see the same files at `docs://skills/<name>/<path>`, with text returned as text and binary
assets returned as base64 blobs.

The optional `/.well-known/agent-card.json` route is different from skill discovery. It appears
only when `agent.a2a` describes a real A2A interface. A preferred ordered
`supportedInterfaces` configuration defaults each entry to protocol `1.0` over `HTTP+JSON`; the
first entry is preferred, and the other core bindings are `JSONRPC` and `GRPC`. The generated card
uses the strict A2A v1 shape. `interfaceUrl` remains only as a deprecated single-interface
shorthand and retains its historical protocol `0.3` default. The preferred interface-array form
requires explicit A2A skills; it does not turn published Agent Skill documents into callable
capabilities automatically. A2A capability entries do not carry Agent Skill document URLs—those
stay in the skill indexes and MCP resources. Do not configure the card merely because the site
exposes docs, Ask AI, or MCP.
...
scaffold one public forwarder each, so the aliases share the same docs server handler. The separate,
opt-in A2A document remains at `/.well-known/agent-card.json`.
...
- which skills and companion files are published, including modern and legacy discovery routes
- whether a separately implemented A2A service publishes an Agent Card
...
CLI install command. This root site skill remains available and is also included in discovery when
`agent.skills` publishes additional reusable skills.
