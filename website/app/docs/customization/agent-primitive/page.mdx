...
<Callout type="info" title="Same URL, different representation">
  ...
  `ETag`, `Content-Language` when a locale is known, and `Last-Modified` when page freshness is known.
  Conditional requests can reuse those validators and receive HTTP `304` without another body.
