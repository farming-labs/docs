...
<Callout type="info" title="Sending events to Docs Cloud">
  This page covers the runtime event stream and optional custom sinks. Connected Docs Cloud projects
  send events to Cloud automatically, store them in Cloud, and expose dashboard views. For the hosted
  pipeline, project IDs, and ClickHouse/Postgres storage, see
