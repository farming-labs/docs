...
agent:
  ...
  outcome: Every tested docs page resolves an absolute OG image URL whose response is a valid social preview image.
  appliesTo:
    framework: nextjs
    version: ">=0.2.60"
    package:
  ...
  files:
    ...
    - public/og.png
  sideEffects:
    - Dynamic generation adds an image endpoint that runs when crawlers request a preview.
  verification:
    - description: Fetch the generated metadata and its og:image URL for a representative docs page.
      expect: The URL is absolute and the image request returns HTTP 200 with an image content type.
  rollback:
    - Restore the previous og configuration and remove the dynamic route or static image introduced by the change.
  failureModes:
    - symptom: Social previews use a relative or unreachable image URL.
      resolution: Configure an absolute sitemap, llms.txt, robots, AI docs, or site base URL and retest generated metadata.
    - symptom: The dynamic image route throws while rendering.
      resolution: Limit the image component to fonts, layout, and CSS supported by the selected image renderer.
---
