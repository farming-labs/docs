...
- `read_page` accepts either a slug like `installation` or a full docs path like `/docs/installation`; pass `section` for one heading and `maxChars` to cap the response
...
- `get_context` deterministically selects matching sections, applies optional `framework`, `version`, and `locale` scopes, and reports source anchors plus character/token accounting
...
`tokenBudget` defaults to `4000`, accepts values from `256` through `32000`, and uses a
deterministic four-characters-per-token estimate. The response reports both the requested budget
and the exact characters used. Returned context never exceeds `tokenBudget * 4` characters, and its
reported token estimate never exceeds the requested budget, so clients can make their own
tokenizer-specific decisions.
