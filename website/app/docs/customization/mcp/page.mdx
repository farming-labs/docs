...
This repo includes a one-command MCP smoke test in the Next example.
...
That helper exercises the full built-in MCP surface:
...
It also performs the required MCP `initialize` handshake and closes the session when the test ends.

If you need to point at a different running server:

```bash title="terminal"
DOCS_MCP_URL=http://127.0.0.1:4023/api/docs/mcp pnpm --dir examples/next test:mcp
```
