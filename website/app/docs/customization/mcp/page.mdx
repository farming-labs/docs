...
- **Facets tool:** `list_search_facets` is also enabled by default. Disable it with `mcp.tools.searchFacets: false`.
...
`read_pages` accepts up to 20 page paths and applies one shared conservative token budget, which
reduces MCP round trips without letting a batch grow without bounds. `submit_feedback` uses the
same `{ context?, payload }` envelope as the HTTP feedback endpoint and validates `payload` against
`feedback.agent.schema` before calling `feedback.agent.onFeedback`.
...
The public docs MCP remains read-only. Run the separate local authoring server when an agent needs
...
Its workflow tools cover status, branch creation, hash-checked reads and writes, machine-readable
page preview, `docs doctor`, Git diff review, and recurring-feedback analysis. Writes are restricted
to `.md`, `.mdx`, and `.svx` files inside `contentDir`; overwriting a file requires the SHA-256 from
the preceding read, which prevents a stale agent from replacing newer work.

Draft-PR publishing is absent by default. Register it only for an operator-controlled session:

```bash
pnpm exec docs mcp author --branch-prefix docs/ --base-branch main --allow-publish
```

The publishing tool requires explicit confirmation, rejects dirty files outside `contentDir`,
requires a matching branch prefix and authenticated GitHub CLI, then commits docs-only changes,
pushes the branch, and opens a draft PR. `createDocsAuthoringMcpServer()` is also exported from the
server entry for teams that place this distinct surface behind authoring-scoped OAuth. It is never
wired into `/mcp`, `/.well-known/mcp`, or a framework public forwarder.

<Callout type="warning" title="Keep authoring separate">
  Do not expose the authoring server through the public docs MCP route. Use local stdio or a
  separately authenticated transport with an authoring-specific OAuth scope.
</Callout>
...
The fallback is triggered whenever the configured `search.endpoint` resolves to the same server as the incoming request **and** the endpoint path matches one of the built-in MCP aliases (`/mcp`, `/.well-known/mcp`, `/api/docs/mcp`) or the custom `mcp.route` value set in `docs.config`. When the match is detected, `search_docs` performs direct simple search instead of issuing a loopback HTTP call to itself. External MCP search endpoints — for example `https://other-service.example.com/mcp` or a path like `/external/search/mcp` that does not match any configured MCP route — are not affected and continue to proxy normally.
## Stdio transport
...
- `list_page_sections` returns body-free section discovery metadata (ids, headings, levels, parent ids, line ranges, token estimates, and markdown URLs) for a given page
- `get_navigation` returns the docs tree in a readable text outline

- `list_search_facets` returns facet values for `framework`, `version`, `package`, and `tags`. Three optional inputs refine the request: `facet` (one of `framework | version | package | tags`) pins the response to a single field and enables cursor-based pagination; `limit` (1–100) caps the number of values returned; `cursor` is the opaque continuation token from a previous response. When the response includes `hasMore: true` on a facet, pass that facet's `nextCursor` value as `cursor` (with the same `facet` argument) to retrieve the next page of values.
...
- `hydrate_content_changes` is called after `list_content_changes` to retrieve the full section bodies and deletion tombstones for a batch of changed documents; it accepts a `tokenBudget` parameter (default `5000`; min `4`; max `32000`) that caps the total hydrated content returned. Enable it with `mcp.tools.hydrateContentChanges: true` in `docs.config.ts`.
