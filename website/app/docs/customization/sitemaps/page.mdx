...
At runtime the docs server applies an additional, higher-priority source before falling back to the filesystem:

1. the date from the bundled `.farm-docs-last-modified.json` manifest (when present)
2. the last git commit date for the page source file (CLI generation)
3. the filesystem modified date for the page source file
4. no date, if none of the above is available

The bundled manifest is a versioned JSON file with the shape `{ version: 1, pages: Record<path, ISO-date> }`. When the docs server finds this file in the content directory, it reads the manifest-provided date for each page first and only falls back to `stat.mtime` when no manifest entry exists. This makes the last-modified date stable across deployments and CI runs, because the bundled dates reflect the original Git history rather than the time files were copied during a redeploy.

For preloaded adapters (Astro, SvelteKit, Nuxt, TanStack Start), include the manifest alongside your content files so the runtime can read it in serverless and static-friendly environments:

```ts title="docs.server.ts"
const contentFiles = import.meta.glob(
  ["/docs/**/*.{md,mdx}", "/.farm-docs-last-modified.json"],
