...
When enabled, the title is added before the rest of the content:

- `format: "markdown"` — prepends `# Title` followed by a blank line.
- `format: "text"` — prepends the plain title followed by a newline.

If the content already begins with the title, it is not duplicated.
