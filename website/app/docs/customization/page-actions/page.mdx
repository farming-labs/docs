---
title: "Page Actions"
description: 'Add copy, LLM, MCP connection, and Agent Skills actions to docs pages'
...
agent:
  tokenBudget: 900
  task: Configure copy, Open in LLM, MCP connection, and Agent Skills actions for docs pages.
  outcome: The selected actions appear in the intended position and produce canonical Markdown, provider URLs, or copyable agent setup instructions.
  appliesTo:
...
This adds the content actions plus **Connect MCP** and **Install skills** setup controls.
...
### `pageActions.connectMcp`

Show one-click, copyable MCP setup for the raw endpoint, Claude Code, Cursor, VS Code, and Codex.
Relative endpoints are resolved against the deployed docs origin.
...
### `pageActions.installSkills`

Read the published `/.well-known/agent-skills/index.json` catalog and offer a copyable Skills CLI
command. You can override the index or command when the docs are hosted behind a proxy.
...
pageActions: {
  installSkills: {
    ...
    command: "npx skills add https://docs.example.com",
  },
}
```

Both actions are opt-in. Keep `mcp` and `agent.skills` enabled so the copied setup resolves to live
...
Each of these built-in presets automatically renders a monochrome provider icon in the dropdown — no `icon` field is required. You only need to supply an `icon` when using a custom `urlTemplate` provider or when you want to override the default mark for a named preset.
