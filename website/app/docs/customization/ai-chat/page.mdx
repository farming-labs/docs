...
# NEXT_PUBLIC_DOCS_CLOUD_PROJECT_ID and NEXT_PUBLIC_DOCS_CLOUD_API_KEY are also accepted as fallbacks.
```
...
| --------------------- | ------------ | ----------- |
| `PUBLIC_DOCS_CLOUD_PROJECT_ID` plus `PUBLIC_DOCS_CLOUD_API_KEY` or another browser-safe `cloud.apiKey.env` (or `NEXT_PUBLIC_*` fallbacks) | The browser posts directly to `/v1/projects/{projectId}/knowledge/ask` with the project-scoped Docs Cloud API key | You want the lowest-latency Cloud path and the key is safe to expose to the browser |
...
For staging or self-hosted Cloud APIs, direct browser requests read `PUBLIC_DOCS_CLOUD_URL` (or the `NEXT_PUBLIC_DOCS_CLOUD_URL` fallback).
Server-routed requests read `DOCS_CLOUD_API_URL` first, then `PUBLIC_DOCS_CLOUD_URL`, then `NEXT_PUBLIC_DOCS_CLOUD_URL`.
