...
2. **Your components** — from `docs.config.ts` (overrides built-ins if names match)
...
<Callout type="tip" title="This section is interactive">
Hover or focus the links below to test `HoverLink` directly on the docs site.
</Callout>

<div className="my-6 rounded-xl border border-black/10 bg-black/[0.02] p-5 dark:border-white/10 dark:bg-white/[0.02]">
  <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-black/45 dark:text-white/45">
    Interactive demo
  </div>
  <p className="mb-0 mt-3 text-sm leading-7 text-black/65 dark:text-white/65">
    Explore the{" "}
    <HoverLink
      href="/docs/installation"
      title="Installation"
    ...
    <HoverLink
      ...
      title="Configuration"
      description="See every top-level docs.config option for themes, nav, metadata, components, and search."
    ...
    <HoverLink
      ...
      description="Build and share a custom theme with your own colors, typography, and UI tokens."
      linkLabel="Open theme guide"
    >
      theme creation
    </HoverLink>
    .
