...
Add `--dry-run` alongside `--agent --fix` when you want to preview which `agent.md` files would be refreshed without actually writing them to disk. The report lists each file that would change with a status of `would-apply`, but no files are written.
