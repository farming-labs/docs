...
Use `docs doctor --agent --fix` when you want the doctor to automatically repair issues it finds rather than only reporting them. Fix mode re-runs agent doc compaction for stale generated `agent.md` files and fills in token-budget missing outputs. Each repair is reported individually with a status of `applied` when the fix was written successfully, or `skipped` when the file was already up to date or the fix could not be applied safely. `--fix` only takes effect when combined with `--agent`; running it with `--site` or without a mode flag is not supported.
