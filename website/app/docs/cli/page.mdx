...
- **Fresh project** — Bootstrap a new app from a template. The CLI asks you to pick a framework (Next.js, TanStack Start, Nuxt, SvelteKit, or Astro), then a project name, creates the folder, clones the template, and runs install.
...
Upgrade all `@farming-labs/*` docs packages to the latest version. Run from your project root; the CLI **auto-detects** your framework from `package.json` and upgrades the right packages for Next.js, Nuxt, SvelteKit, and Astro.
...
> TanStack Start is currently supported by `init`, but not by `upgrade`.
...
| TanStack Start | `@tanstack/react-start` in dependencies | Supported | Not yet |
...
<Tabs items={["Next.js", "TanStack Start", "SvelteKit", "Astro", "Nuxt"]}>
  ...
  <Tab value="TanStack Start">
    ...
    2. **Asks for a theme** — built-in themes or **Create your own theme** (then prompts for theme name; press Enter for default `my-theme`)
    3. **Asks for the entry path** — Default is `docs` (press Enter to accept)
    4. **Skips the built-in i18n scaffold** so the generated TanStack routes stay minimal and working
    5. **Asks about path aliases** — Use `@/` alias or relative paths for imports
    6. **Generates files:**
       - `docs.config.ts` — Full config with your selected theme
