...
markdown, `llms.txt`, sitemap, skill, MCP, and agent discovery routes.
...
and local agent-readiness signals. With `--url`, it also sends real requests to the hosted site and
adds hosted checks to the report.
...
For MCP, the doctor performs a Streamable HTTP `initialize` request, checks for an
`mcp-session-id`, sends `tools/list`, and expects the built-in docs tools:
...
Hosted checks raise the agent report from `105` possible points to `145` possible points. The JSON
...
`hosted-markdown`, and `hosted-mcp`.
