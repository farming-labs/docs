...
Use `--strict` in CI when warnings should fail the job. Strict mode sets a non-zero exit code when any doctor check reports `warn` or `fail`, and it can be combined with `--json` for machine-readable output.
