...
| Flag                  | Description                                                                 |
| --------------------- | --------------------------------------------------------------------------- |
| `--template <name>`   | Clone an example and get started: `next`, `nuxt`, `sveltekit`, or `astro`  |
| `--theme <name>`      | Skip theme prompt (e.g. `--theme darksharp`)                                |
| `--entry <path>`      | Skip entry path prompt (e.g. `--entry docs`)                                |

**Starting from scratch?** Use `--template` in an empty folder — the CLI clones the corresponding example from the repo (e.g. `examples/next`) into the current directory and runs install. No prompts; then run your dev server and you're set.
