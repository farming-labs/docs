...
- **`agent export`** — materialize the complete machine-readable surface for static hosting
...
```bash title="terminal"
pnpm dlx @farming-labs/docs search sync --typesense --sync-namespace product-docs
```
...
Pass `--site-url` when you want the sync command to embed a canonical public URL in the indexed documents (overrides any base URL derived from config):
...
The generated allow-list covers wildcard agent-skills routes (`/.well-known/agent-skills/*`) and the legacy skills namespace (`/.well-known/skills/index.json`, `/.well-known/skills/*`). Two rules are conditional:

- `Allow: /.well-known/api-catalog` is **omitted** when `llmsTxt.apiCatalog: false` or `staticExport: true`.
- `Allow: /.well-known/agent-card.json` is **included** only when `agent.a2a` is configured.
...
### Export a static Agent Bundle

Use `agent export` when a static deployment needs the same machine-readable files that a runtime
adapter normally serves:
...
- public `AGENTS.md` and `AGENT.md` aliases
- configured XML/Markdown sitemaps and `robots.txt`
- `.farming-labs/sitemap-manifest.json` for stable page freshness during the framework build
- `/.well-known/agent-bundle.json` and `.farming-labs/agent-bundle-manifest.json`, containing
  deterministic SHA-256 hashes for the exported files

Every file entry in the bundle manifest now includes three additional fields derived from the exact exported bytes:

| Field | Description |
| ----- | ----------- |
| `etag` | Strong HTTP entity tag in the form `"<64-hex-char sha256>"` — the same value served as `ETag` by live adapters |
| `contentDigest` | RFC 9530 `Content-Digest` field value in the form `sha-256=:…:` (base64-encoded SHA-256) |
| `lastModified` | Optional ISO-8601 source modification time when the exporter can derive one (e.g. from the last git commit date for tracked files) |
The exported artifact is static by definition. Its discovery document disables server-only search,
MCP, feedback, API reference, and OpenAPI capabilities even when the application config is otherwise
server-rendered.

The static bundle also always advertises `capabilities.contentChanges: false` and omits the `contentChanges` API endpoint, because the runtime change feed requires a live server and is unavailable in static exports. Page freshness uses tracked git commit dates; untracked pages omit `last_updated`
instead of hashing checkout-specific filesystem mtimes.

Add the read-only check to CI when generated files are committed:

```bash title="terminal"
pnpm exec docs agent export --check
```

`--public` is required for a write. `--check` implies the public/static target but never modifies
files. Existing native page, llms.txt, discovery, skill, public `AGENTS.md`, and user-owned
`robots.txt` overrides remain in control.
The exporter never copies the repository-root `AGENTS.md` into the public directory; use
`agents generate` when publishing those repository instructions is intentional. Generated writes
use same-directory temporary files and atomic renames, and the manifest lets later exports safely
remove artifacts that the previous bundle owned but no longer generates. Output roots are checked
through existing symlinks before any write or stale deletion. If an obsolete managed file was
edited, the exporter preserves it as an orphan and `--check` continues to fail until you remove the
file manually and rerun the export.

For a full static build, run the export before the framework build:
...
Use `docs skills scaffold` to compile structured page `agent` contracts into an installable, progressively disclosed Agent Skill. The command reads actionable pages from the configured docs source, writes one compact `SKILL.md` routing file and one focused `references/*.md` file per selected page into a `skills/<name>/` directory, and works offline and deterministically.
...
# Write (or update) the skill directory
pnpm exec docs skills scaffold

# Fail if the generated skill files are stale — useful in CI
pnpm exec docs skills scaffold --check
```

Default behavior:

- derives the skill name from `nav.title` in `docs.config`
- writes output to `skills/<name>/`
- includes all actionable pages (pages that define a structured `agent` contract)

Flags:

| Flag | Description |
| ---- | ----------- |
| `--name <name>` | Override the derived skill name. Accepts a positional name as well. |
| `--output <dir>` | Write the skill to a custom directory instead of `skills/<name>/`. |
| `--include <slug>` | Repeatable. Restrict scaffolding to the listed page slugs or paths. |
| `--config <path>` | Use a custom docs config path (e.g. `src/lib/docs.config.ts`). |
| `--dry-run` | Print what would be written without touching the filesystem. |
| `--force` | Overwrite existing skill files even when they appear up to date. |
| `--check` | Exit non-zero if the generated output would differ from what is already on disk. |

Output shape:

- `skills/<name>/SKILL.md` — a compact routing file that names the skill and links to each reference
- `skills/<name>/references/<slug>.md` — one file per selected actionable page, containing the authored task, expected result, applicability, prerequisites, files, commands, side effects, verification, recovery, rollback, and canonical source, but not the full page prose

Once generated, publish the skill through `agent.skills` in `docs.config.ts` so it is indexed by the runtime, Static Agent Bundle, and MCP:
...
Add the check to CI when generated skill files are committed:

```json title="package.json"
{
  "scripts": {
    "check:skills": "docs skills scaffold --check"
  }
}
```
...
- duplicate, boilerplate, or generic `<Agent>` blocks across the full docs corpus
- prerequisites, observable results, recovery guidance, and framework/version applicability
- documented command health and related-page coverage
- config loading confidence and drift between discovery, resolved config, contracts, and schema
- configured golden tasks for retrieval, citations, version selection, answers, examples, and budgets

- adversarial safety suites within configured golden tasks (prompt injection, poisoned citations, authenticated-content leakage, freshness, framework-version conflicts, deleted-section tombstones, and query variants)
...
reports retrieval usefulness as `unmeasured` and awards no golden-task points. See
[Golden agent evaluations](/docs/configuration#golden-agent-evaluations) for the task format.
They use the local `mcp-context` surface with no implicit model, network, or runtime execution by
default. Opt-in `configured-search` and `ask-ai-context` surfaces fail closed when their configured
...
Useful Agent blocks: 35/35 • 18/18 actionable pages task-complete
Golden tasks: 4/4 passed (100/100)
Golden task quality: 4/4 passed (100/100)
Evaluation coverage: measured (12/12 task-dimensions, 100%)
...
PASS Agent context usefulness (15/15)
  35/35 Agent blocks are specific and non-repetitive (0 duplicate, 0 boilerplate, 0 generic).
PASS Golden agent tasks (15/15)
  4/4 golden tasks passed with 100/100 average score.
```txt
PASS Golden agent tasks (15/15)
  4/4 golden tasks passed with 100/100 average score. 2/2 adversarial safety suites passed.
```

When safety suites are configured on tasks, the doctor appends a summary such as `2/2 adversarial safety suites passed` or `no adversarial safety suites configured` to the golden-tasks finding.
...
```json title="~"
{
  ...
  "evaluations": {
    ...
    "coverage": {
      ...
      "measuredTaskDimensions": 5,
      "totalTaskDimensions": 12,
      "coveragePercent": 42
    },
    "taskCount": 4,
    "passedTaskCount": 3,
...
Use `--ci` when running inside GitHub Actions to emit file/line workflow annotations for each
finding directly to the job log. Because `--ci` prints annotations to stdout, pair it with
`--json-output <path>` when the same run must also persist a machine-readable report:

```bash title="terminal"
pnpm exec docs doctor --agent --ci --json-output .farming-labs/doctor.json
```

`--json-output <path>` writes the JSON report to a file and accepts both space-separated
(`--json-output <path>`) and equals-sign (`--json-output=<path>`) forms. It can be combined with
`--ci`, `--json`, `--strict`, or any other doctor flag.
### Hosted agent checks
...
When `agent.contentChanges` is configured in `docs.config`, the stdio server also registers the `list_content_changes` tool and its associated polling resources for body-free document-change detection.
...
with `--ci`. The review also uses the full docs corpus to catch copied Agent boilerplate on a
changed page, statically validates commands and related routes, reports low-confidence config
loading or public-surface drift, and runs the same configured golden tasks as `docs doctor`.
The same offline defaults and explicit network, answer-provider, and example-execution opt-ins apply.

> When golden tasks are configured, `docs review` also checks adversarial safety suites. Without tasks, retrieval, citations, version selection, adversarial safety, executable examples, and token usage are all unmeasured.
...
JSON reports use `status: "measured"` with a numeric score. When review is disabled, they use
`status: "disabled"` and `score: null` instead of presenting an unrun review as 100/100.
...
| `shadcn`       | Compact neutral shell inspired by the shadcn/ui documentation |
...
| `--theme <name>`        | Skip theme prompt: `fumadocs`, `darksharp`, `pixel-border`, `colorful`, `darkbold`, `shiny`, `ledger`, `shadcn`, `greentree`, `concrete`, `command-grid`, `hardline` |
