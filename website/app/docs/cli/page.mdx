...
3. The CLI creates the folder, clones the template with `degit`, and runs install using the package manager you chose. You then run `cd <name>` and start the dev server with that same package manager.
...
6. The CLI generates config, layout, and sample pages, then detects the package manager from a lockfile when possible. If no lockfile is found, it asks which package manager you want to use before installing dependencies, and can then start the dev server.
...
<Callout type="info" title="Rename to docs.config.tsx when using JSX">
