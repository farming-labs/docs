...
Use `docs doctor --agent --fix` when you want the doctor to automatically repair issues it finds instead of only reporting them. In fix mode, the command refreshes stale generated `agent.md` files and regenerates pages that are missing token-budget outputs, then reports the results in a `fixes` array on the JSON report.
...
```json title="~"
{
  ...
  When `--fix` is passed, the report also includes a `fixes` array:

  ```json title="~"
  {
  "fixes": [
    {
      "id": "stale-agent-md:docs/installation",
      "title": "Refreshed stale agent.md for docs/installation",
      "status": "applied",
      "detail": "Recompacted from updated page source."
    },
    {
      "id": "missing-token-budget:docs/quickstart",
      "title": "Generated missing token-budget output for docs/quickstart",
      "status": "skipped",
      "detail": "No compact config found; skipped."
    }
  ]
  }
  ```

  Each entry in `fixes` contains:

  | Field | Type | Description |
  | ----- | ---- | ----------- |
  | `id` | `string` | Unique identifier for the fix, including the affected page path |
  | `title` | `string` | Human-readable description of what was attempted |
  | `status` | `"applied" \| "skipped"` | Whether the fix was written (`applied`) or left unchanged (`skipped`) |
  | `detail` | `string` | Additional context about why the fix was applied or skipped |
```
