...
Add `--dry-run` together with `--agent --fix` when you want to preview which `agent.md` files would be refreshed or generated without actually writing any files to disk. The command reports the same fix plan — including which pages are stale or missing — but leaves all files unchanged.
