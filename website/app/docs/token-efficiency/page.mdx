...
Built-in AI chat with retrieval-augmented generation — two lines in your config:
...
No separate API routes, no vector databases, no embedding pipelines. Search indexing, context retrieval, and streaming responses are handled internally.

See the [AI Chat docs](/docs/customization/ai-chat) for details.

## Summary

`@farming-labs/docs` keeps the framework lean — one config file, minimal routing, CLI-generated scaffolding — so you and your AI tools spend less time on plumbing and more time on content. And it does this without sacrificing flexibility: themes, AI chat, search, custom components, and multi-framework support are all still there, just configured from one place.
