...
<Callout type="info" title="Markdown via content negotiation">
  ...
  when the page locale is known. Every successful response has an `ETag`, and pages with known
  freshness also include `Last-Modified`; matching `If-None-Match` or `If-Modified-Since` requests
  receive HTTP `304`.
  ...
  When an `Accept` header lists both HTML and markdown, the shared handler honors their `q` values;
  the representation with the higher quality wins, with an explicit tie favoring markdown.
