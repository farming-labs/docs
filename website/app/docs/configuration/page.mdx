...
- Page frontmatter `related` is rendered as a comma-separated markdown metadata line beside `Description`, so agents can discover adjacent pages without scraping the UI
