...
<Callout type="info" title="Renderer choice">
  The default renderer is **Scalar**. In **Next.js** you can also set
  `apiReference.renderer: "fumadocs"` to use the Fumadocs OpenAPI experience instead.
</Callout>
...
Next.js can switch the API reference UI to the Fumadocs renderer:
...
| `renderer`  | `"scalar" \| "fumadocs"` | `"scalar"` | UI renderer to use. `fumadocs` is currently supported in Next.js only |
