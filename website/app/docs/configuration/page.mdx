...
Configure `agent.a2a` only when `interfaceUrl` implements a real A2A interface. A docs API, Ask AI
route, or MCP endpoint does not satisfy that contract. The protocol defaults to `0.3` over
`HTTP+JSON`; set `protocolVersion` and `protocolBinding` only when the implemented interface
differs. When configured, the framework publishes `/.well-known/agent-card.json`; otherwise it
leaves the route absent.
