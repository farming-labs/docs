...
agent:
  tokenBudget: 1800
  task: Change docs.config safely for the selected framework and verify the resolved public behavior.
  outcome: The configuration module loads successfully and the requested docs capability is reflected in diagnostics and discovery.
  appliesTo:
    framework: [nextjs, tanstackstart, sveltekit, astro, nuxt]
    version: ">=0.2.60"
    package: "@farming-labs/docs"
  prerequisites:
    - Identify the project framework and its documented docs.config location.
    - Start from an existing defineDocs configuration and preserve framework-specific entry, contentDir, and nav requirements.
  files:
    - docs.config.ts
    - docs.config.tsx
  ...
  commands:
    - run: pnpm exec docs doctor --agent
      description: Evaluate the config and compare it with the generated agent surface.
  verification:
    - run: pnpm exec docs doctor --agent
      expect: Config loading confidence and discovery/config/schema consistency pass.
    - description: Read /api/docs?format=diagnostics and confirm the requested capability is enabled without configuration issues.
  rollback:
    - Restore the previous defineDocs option and any framework route wiring changed with it.
  failureModes:
    - symptom: The config cannot be evaluated.
      resolution: Use docs.config.tsx when the file contains JSX and fix unresolved imports or environment-dependent top-level code.
    - symptom: Config and public discovery disagree.
      resolution: Run the agent doctor and align the adapter route wiring with the resolved docs.config values.
---
...
| `agent`       | `DocsAgentConfig`              | —               | Compaction, golden evaluations, reusable skills, and optional A2A metadata |
...
Use metadata on the code fence to tell agents and validators how a snippet should be treated:
...
Small runnable blocks make the check easy to dogfood:

```json title="agent-context.json" runnable
{
  "framework": "nextjs",
  "packageManager": "pnpm",
  "agentReady": true
...
```bash title="codeblocks-smoke.sh" runnable
test "docs" = "docs"
echo "shell ok"
```
...
The planner never writes secrets into docs. The `env` map means the snippet can keep using
`process.env.OPENAI_API_KEY`, while validation injects that runtime variable from
...
- Successful markdown responses include canonical `Link` and `Content-Location` response headers, plus cache validators and locale metadata when available
- In Next.js, sending an unambiguous `Accept: text/markdown` request to `/docs/<slug>` returns the same markdown response; mixed ranges that also advertise `text/html`, `text/*`, or `*/*` remain HTML
...
<Callout type="info" title="Markdown via content negotiation">
  ...
  that send an unambiguous `Accept: text/markdown`, send `Signature-Agent`, identify as a known AI
  user agent, or match a conservative bot-like agent heuristic receive the same machine-readable
  output as `/docs/installation.md`. The request is resolved by the shared `/api/docs` handler, so
  custom apps should keep one docs API route instead of adding a second markdown-only wrapper.
  Other adapters can negotiate by `Accept`, or use the `.md` URL or API format route directly.
  ...
  `Content-Location` identifies the selected `.md` representation. `Content-Language` is included
  when the page locale is known. Every successful response has an `ETag`. `Last-Modified` is only
  included when the adapter knows the exact source timestamp; a date-only `last_updated` value is
  not promoted to midnight. Matching `If-None-Match` or `If-Modified-Since` requests receive HTTP
  `304` when the corresponding validator is available.

  The markdown body starts with YAML frontmatter. Agents can read `canonical_url`, `markdown_url`,
  `title`, `description`, and `last_updated` before parsing the page body.
  ...
  The shared handler and non-Next request handlers honor `q` values when an `Accept` header lists
  both HTML and markdown. Next.js canonical-page negotiation is implemented as a static rewrite, so
  it intentionally does not attempt arbitrary numeric `q` comparisons: any `text/html`, `text/*`,
  or `*/*` range keeps the canonical URL on HTML. Use `/docs/installation.md` or
  `/api/docs?format=markdown&path=installation` when the Markdown representation must be exact.

  That means a CDN can cache the HTML and markdown versions separately even though the URL is the
  same:

| `apiCatalog` | `boolean` | `true` | When `false`, suppresses `Allow: /.well-known/api-catalog` from the generated `robots.txt` policy and removes that route from discovery links. Has no effect on hashed Agent Skills routes. |
  ...
  If a markdown request misses, the recovery response is still markdown and returns HTTP `404`
  unless a very high-confidence match redirects to the corrected `.md` page. The body lists closest
  matches, recovery steps, the agent discovery spec, search endpoint, and sitemap routes so agents
  can recover instead of stopping at a plain `Not Found`.
</Callout>
...
`/docs-map/.well-known/sitemap.md`. The default `/docs/sitemap.md` alias is emitted only when
`routePrefix` is empty. See [Sitemaps](/docs/customization/sitemaps) for static export, manifest,
`lastmod`, and output examples.
...
Two allow rules are conditional: `Allow: /.well-known/api-catalog` is omitted when `llmsTxt.apiCatalog: false` or `staticExport: true`, and `Allow: /.well-known/agent-card.json` is included only when `agent.a2a` is configured.
...
Use the exact lowercase `explain=true` opt-in when an agent needs to understand why a result was
selected:
...
Each result then includes a bounded `explanation` with:

- matched query terms and their title, section, description, content, or URL fields
- the final provenance scope plus framework, version, package, and tag filter decisions
- unresolved or unverifiable ambiguity, or the filter that resolved it
- the one-based rank, ranking strategy, and concrete ranking reasons

The default and `explain=false` omit this field, including from legacy array responses. For
Typesense, Algolia, MCP, and custom adapters, the explanation identifies provider-owned ordering
without claiming access to private provider ranking internals.

The built-in MCP tool exposes the same behavior:

```json title="search_docs arguments"
{
  "query": "deploy",
  "audience": "agent",
  "explain": true
...
Streamable HTTP MCP is public by default. Add `security.authenticate` only when the endpoint needs
access control. To support OAuth-aware MCP clients, configure `protectedResource` alongside the
callback:
...
`authorizationServers` must be non-empty and contain HTTPS issuer URLs without query or fragment
(loopback HTTP is accepted for development). `scopesSupported` is discovery metadata, while
`requiredScopes` is enforced against the principal returned by `authenticate`. A `null` result
receives a `401` Bearer challenge that links to public RFC 9728 metadata; a principal missing a
required scope receives `403 insufficient_scope`. If the callback returns a Web `Response`, that
response is preserved and must include its own challenge when needed.
The framework does not issue or validate tokens; the callback must validate signature, issuer,
expiry, the exact callback `resource` audience, and derive scopes from the verified token. The
callback `request` retains the real incoming URL and query string.

The default metadata routes are `/.well-known/oauth-protected-resource/mcp`,
`/.well-known/oauth-protected-resource/.well-known/mcp`, and
`/.well-known/oauth-protected-resource/api/docs/mcp`. The last path follows a custom `mcp.route`.
Protected-resource discovery is inactive without `authenticate`, and stdio is unaffected. Use HTTPS
for hosted MCP, authorization-server, and authentication-documentation URLs.
Treat each MCP alias as a distinct OAuth resource identifier and validate token audiences against
the exact endpoint used by the client.
On Next.js, do not combine protected MCP with `basePath`; host the protected routes at the origin
root or an edge proxy. Inline MCP objects are detected automatically. For imported, spread, or
shorthand MCP configuration, import `docsConfig` in `next.config.ts` and call
`withDocs(nextConfig, docsConfig)` so routing and the `basePath` check use the live object.
...
- `list_tasks` filters actionable contract summaries by query/framework/version/package; `read_task` returns the full normalized contract as structured output plus a JSON text fallback
- `list_docs` and `list_pages` expose only `hasContract`, `task`, `outcome`, and `appliesTo` in their contract summaries
...
- `get_context` returns deterministic section-level context with source anchors and an explicit conservative UTF-8 byte ceiling for the complete assembled response

Framework notes:

- **Next.js:** `withDocs()` auto-generates the default `/api/docs/mcp` route and public `/mcp` plus `/.well-known/mcp` rewrites

When the Farm application lives inside a monorepo or needs to compile documentation from a shared
location outside its own directory, pass `contentRoots` to `withDocs`:
...
`contentRoots` accepts an array of paths relative to the Farm application root. Each resolved path
is added to the dev server's `server.fs.allow` list so the MDX plugin can compile content from
repository-owned or shared documentation directories.
- **TanStack Start:** the current `init` scaffold adds one `src/routes/$.ts` public forwarder for `/api/docs/mcp`, `/mcp`, `/.well-known/mcp`, `llms.txt`, agent discovery, and `.md` routes
- **SvelteKit:** the current `init` scaffold adds one `src/hooks.server.ts` public forwarder for `/api/docs/mcp`, `/mcp`, `/.well-known/mcp`, `llms.txt`, agent discovery, and `.md` routes
- **Astro:** the current `init` scaffold adds one `src/middleware.ts` public forwarder for `/api/docs/mcp`, `/mcp`, `/.well-known/mcp`, `llms.txt`, agent discovery, and `.md` routes
- **Nuxt:** the current `init` scaffold adds one `server/middleware/docs-public.ts` public forwarder for `/api/docs/mcp`, `/mcp`, `/.well-known/mcp`, `llms.txt`, agent discovery, and `.md` routes
...
Docs Review is enabled by default. During `next dev` or `next build`, `withDocs()` creates
`.github/workflows/docs-review.yml` when the workflow does not already exist. The generated workflow
is intentionally tiny: it calls the hosted reusable workflow at
`farming-labs/docs/.github/workflows/docs-review-reusable.yml@main` and passes the docs config path,
package manager, check name, and working directory as inputs. In monorepos, the file is written at
the repository root and the job runs from the docs app directory.
...
The generated wrapper only triggers on docs-related PR changes such as `docs.config.*`,
`app/docs/**`, `src/app/docs/**`, `content/docs/**`, and the workflow files themselves. By default
it runs in `warn` mode, posts GitHub annotations, uses `docs-review` as the GitHub check name, and
keeps CI green.
...
evaluations and an unmeasured task suite. `agentSkills` reports oversized skill instructions,
broken or deep references, missing compatibility requirements, and scripts without dependency or
validation guidance.
...
When `enabled` is `false`, the configured `default` value (`"light"` or `"dark"`) is applied as a **forced** theme and the switcher is hidden entirely. When the toggle is enabled (the default), the same `default` value is used as the **initial** theme that visitors can still change. Setting `default: "system"` skips forced and default theme derivation in both cases, deferring to the operating-system preference.
...
Set `requireComment: true` to disable the submit button until the user types a non-empty comment:
...
<Callout type="info" title="Feedback is not prompt context">
...
Use `agent.evaluations` to define user-shaped tasks that `docs doctor` and `docs review` can measure.

Golden tasks measure retrieval recall, citation accuracy, version selection, adversarial safety, answer quality, executable examples, and token budget.
The default `mcp-context` surface is local and makes no implicit model, network, or command-execution
request. You can explicitly measure the configured search pipeline, the production Ask AI context,
an actual answer provider, or runtime-verified examples when those checks are useful.
  ...
  agent: {
    evaluations: {
      ...
      tasks: [
        {
          id: "next-16-install",
          query: "Install the docs framework in a Next.js 16 app",
          filters: {
            framework: "nextjs",
            version: "16",
          },
          expect: {
            ...
            requiredCitations: ["/docs/installation"],
...
Top-level `tokenBudget` and `topK` are defaults; either can be overridden on an individual task.
`requiredCitations` defaults to `relevantSources`, and `allowedSources` can name legitimate
supporting pages that should not reduce citation precision. Source values may be canonical page or
section URLs. `filters` constrain retrieval, while `expect.scope` independently asserts that the
returned sources selected the right framework, version, or locale. This distinction catches a
provider that returns a plausible result for the wrong target instead of hiding that result before
evaluation.

Choose the surface globally with `agent.evaluations.surface` or override it on one task:

- `"mcp-context"` is the default built-in context builder and stays local.
- `"configured-search"` queries the project's actual `search` provider and evaluates only its
  returned order and citations, without local-result supplementation or diagnostic reranking.
- `"ask-ai-context"` runs the production Ask AI retrieval and context-assembly path, including its
  source headers and truncation behavior.
...
Add `expect.safety` to a task to assert that the agent's retrieval and answer pipeline resists adversarial inputs. These fields are evaluator-only and must never contain real secrets or private data.
  ...
  agent: {
    evaluations: {
      tasks: [
        {
          id: "safe-retrieval",
          query: "Configure safe retrieval",
          expect: {
            relevantSources: ["/docs/safe-retrieval"],
            safety: {
              promptInjection: {
                markers: ["SAFETY_INJECTION_CANARY"],
                forbiddenAnswerText: ["ignore the user"],
              },
              poisonedCitations: true,
              authenticatedContent: {
                forbiddenSources: ["/docs/private-control-plane"],
                forbiddenText: ["PRIVATE_AUTH_CANARY"],
              },
              freshness: {
                indexGeneration: "expected-generation-digest",
                sourceDigests: {
                  "/docs/safe-retrieval": "sha256:abc123",
                },
              },
              rejectConflictingFrameworkVersions: true,
              deletedSectionTombstones: true,
              queryVariants: [
                { kind: "rephrased" },
                { kind: "adversarial" },
              ],
...
| ----- | ---- | ----------- |
| `promptInjection.markers` | `string[]` | Canary fragments that must be present in retrieved context |
| `promptInjection.forbiddenAnswerText` | `string[]` | Canary fragments the answer must not repeat. Defaults to `markers` |
| `poisonedCitations` | `boolean` | Assert that retrieval rejects injected citation payloads |
| `authenticatedContent.forbiddenSources` | `string[]` | Protected page or section URLs that public retrieval must not expose |
| `authenticatedContent.forbiddenText` | `string[]` | Stable canary fragments that must not appear in context or answers |
| `freshness.indexGeneration` | `string` | Expected retrieval-index generation digest. Omit to require one consistent generation |
| `freshness.sourceDigests` | `Record<string, string>` | Expected full-document digest keyed by canonical page or section URL |
| `rejectConflictingFrameworkVersions` | `boolean` | Assert that the pipeline rejects sources with conflicting framework version provenance |
| `deletedSectionTombstones` | `boolean` | Assert that deleted-section tombstones are surfaced rather than silently dropped |
| `queryVariants[].kind` | `string` | Additional query variant kind to evaluate, e.g. `"rephrased"` or `"adversarial"` |

### Evaluate actual answers

Answer scoring is opt-in. Add `expect.answer` to a task and configure either a callback or an HTTP
provider. The framework does not choose or call a model when `answer` is omitted.
...
export default defineDocs({
  agent: {
    evaluations: {
      ...
      answer: {
        provider: "http",
        endpoint: process.env.AGENT_EVAL_ENDPOINT!,
        headers: process.env.AGENT_EVAL_TOKEN
          ? { authorization: `Bearer ${process.env.AGENT_EVAL_TOKEN}` }
          : undefined,
        timeoutMs: 10_000,
      },
      tasks: [
...
The HTTP endpoint receives the task id, query, filters, surface, context, and retrieved sources as
JSON, but never receives the expected answer, citations, or callback-only abort signal. It returns
`{ text, citations? }`. HTTP
authentication is optional and explicit through `headers`; header values are never copied into
evaluation reports. The built-in HTTP provider and managed external search requests require
`allowNetwork: true` and use fail-closed provider errors. A
`provider: "callback"` answer instead calls your explicit `run(input)` function and expects the same
answer result shape. That callback is user code, so it may perform only the work you put inside it;
`allowNetwork` does not sandbox or reinterpret it.

### Choose example verification strength

Set `expect.examples[].verification` to the evidence the task must produce:

- `"present"` requires a matching retrieved fence.
- `"syntax"` requires supported static syntax validation. It is the default for runnable examples.
- `"execute"` requires the configured validator to execute the snippet and return `PASS`. It only
  runs when `allowNetwork: true` and `codeBlocks.validate` are explicitly enabled in `"report"`
  mode with a suitable runner.

Examples marked `runnable: false` default to `"present"`. Unsupported syntax, a skipped runner, or a
missing execution opt-in is reported as skipped or failed and never receives passing credit.

An absent, disabled, or empty task suite is deliberately `unmeasured`, not passing. This keeps a
site from receiving usefulness credit merely because no representative tasks were defined. Put
`runnable`, `framework`, `packageManager`, and `title` metadata on expected code fences so example
checks can select the intended snippet deterministically.
...
- optional structured fields include `task`, `outcome`, `appliesTo`, `prerequisites`, `files`,
  `commands`, `sideEffects`, `verification`, `rollback`, and `failureModes`
- structured fields are emitted into machine-readable Markdown, search/Ask AI context, Schema.org
  `HowTo` JSON-LD, and MCP task tools; page-list tools keep only the concise contract summary
- invalid or unknown fields are ignored at runtime and reported by `docs review`; misspelled keys get
  closest-name suggestions such as `verfication` → `verification`
...
- generated contract markers are excluded from compaction input, and rendering preserves a
  handwritten `## Agent Contract`, so repeated compaction/read cycles do not duplicate the section
...
- if inherited `minOutputTokens` would be greater than the page budget, the CLI clamps it down to
  that page budget before calling the compression API
...
Bundles, and MCP. The value can be a string, an array, or `{ paths }`:
...
A path can point to one `SKILL.md`, a skill directory containing `SKILL.md`, or a collection
directory whose descendants contain skill directories. Paths are relative to the project root and
must remain inside its workspace. Publication accepts only non-symlink regular files and
directories. It includes the skill document and safe files below `references/`, `scripts/`, and
`assets/`; other sibling content is not exposed.

Skills with only `SKILL.md` are indexed as `skill-md`. Skills with companion files are packaged as
deterministic `.tar.gz` archives, and the index digest covers the exact indexed artifact bytes.
The original files stay directly available for progressive disclosure. The project-root
`skill.md` remains the backwards-compatible site skill and is published alongside configured
skills.

`docs doctor --agent` checks every configured skill, and `docs review` checks the affected skill
when `SKILL.md`, `references/`, `scripts/`, `assets/`, or the docs config changes. A local reference
must stay inside the skill and resolve to content that publication exposes. Reference Markdown can
be one hop away by default; move second-level detail into a directly linked focused reference.
Token estimates use a dependency-free, code-conscious approximation and can be tuned with
`instructionTokenBudget`.

| Surface | Route or URI |
| ------- | ------------ |
| Agent Skills v0.2 index | `/.well-known/agent-skills/index.json` |
| Indexed skill archive | `/.well-known/agent-skills/<name>.tar.gz` |
| Direct skill files | `/.well-known/agent-skills/<name>/SKILL.md` and `/<companion-path>` below it |
| Legacy index and files | `/.well-known/skills/index.json` and direct files below `/.well-known/skills/<name>/` |
| MCP resources | `docs://skills/<name>/<path>` |

MCP returns text-compatible resources as `text` and binary assets as base64 `blob` values. Running
`docs agent export --public` emits the same indexes, artifacts, direct companion files, and hashes
for static hosts.

### Optional A2A Agent Card

Configure `agent.a2a` only when the advertised URLs implement real A2A interfaces. A docs API, Ask
AI route, or MCP endpoint does not satisfy that contract. Prefer ordered `supportedInterfaces`; the
first entry is the preferred interface. Each interface defaults to A2A protocol `1.0` over
`HTTP+JSON`. The core bindings are `JSONRPC`, `GRPC`, and `HTTP+JSON`; identify a custom binding
with an absolute URI. The deprecated `interfaceUrl` shorthand remains available for a single
interface, with top-level `protocolVersion` and `protocolBinding` applying only to that shorthand.
It retains the historical protocol `0.3` default; set `protocolVersion: "1.0"` explicitly when a
shorthand interface implements v1. Core interfaces and all HTTP card/authentication metadata
require HTTPS outside loopback development. Custom bindings use a secure binding-appropriate
absolute URL such as WSS or MQTTS.

The published `/.well-known/agent-card.json` uses the strict A2A v1 shape: it does not emit the
legacy top-level `url`, `protocolVersion`, or `preferredTransport` fields. Describe only
capabilities, media modes, A2A skills, and optional security metadata that the service really
implements. An A2A skill has no URL; configured Agent Skill documents and artifacts keep their URLs
in the Agent Skills indexes and MCP resources. `supportedInterfaces` requires explicit A2A skills;
automatic projection of published documentation skills remains only for the deprecated shorthand.
When `agent.a2a` is omitted, the Agent Card route is not advertised or generated.
...
agent: {
  ...
  a2a: {
    ...
    description: "Answers implementation questions from the product docs.",
    supportedInterfaces: [
      {
    ...
    skills: [
      {
        ...
        description: "Answers implementation questions using the product docs.",
...
See [Agent Primitive](/docs/customization/agent-primitive#agent-discovery) for the discovery flow and
[API Reference](/docs/reference#docsagentconfig) for the exact types.
...
the versioned Farming Labs manifest schema and format identifiers,
...
- `GET` and `HEAD /.well-known/api-catalog` return the RFC 9727 API catalog
- set `llmsTxt.apiCatalog: false` to remove that catalog route and its discovery links without
  disabling the hashed Agent Skills routes
- `GET` and `HEAD /.well-known/agent-skills/index.json` return the modern hashed Agent Skills index
- `GET` and `HEAD /.well-known/skills/index.json` return the legacy compatibility index; indexed
  artifacts and safe direct files remain available at the routes described above
- dynamic adapters set the catalog media type and discovery `Link` headers; static hosts should
  serve `/.well-known/api-catalog` as `application/linkset+json` with the RFC 9727 profile
- `GET /.well-known/agent.json` is the preferred public agent discovery URL
- `GET /.well-known/agent` is the public fallback discovery URL
- `GET /api/docs/agent/spec` returns the canonical framework discovery document
- the manifest declares the Draft 2020-12 schema at
  `https://docs.farming-labs.dev/schema/agent-manifest.v1.json`; dynamic responses advertise it with
  `rel="describedby"` while preserving `application/json`
- the manifest is a Farming Labs extension; the separate `/.well-known/agent-card.json` route is
  only published when `agent.a2a` configures a real A2A service
- `GET /AGENTS.md` serves a root `AGENTS.md` or `AGENT.md` file when present, and otherwise returns a generated fallback
- `GET /.well-known/AGENTS.md` and `GET /api/docs?format=agents` return the same agent instructions
...
- the discovery JSON advertises `agentSpecDefault`, `agentSpecFallback`, `apiCatalog`,
  `skills.discovery`, `agents.file`, `agents.route`, `skills.file`, `skills.route`, the root and
  section `llms.txt` routes, `robots.route`, and sitemap routes when enabled
...
### Turn recurring feedback into improvements

Store callback payloads as a JSON array/object or JSON Lines export, then cluster recurring failed
tasks locally:
...
The report contains evidence clusters, deduplicated missing context and doc issues, draft GitHub
issues, draft golden tasks for `agent.evaluations.tasks`, and a draft documentation PR title/body.
Only failed, blocked, partial, unclear, or code-reading-dependent reports participate; a cluster
needs two reports by default. Use `--min-occurrences` to raise that threshold. Without `--write`,
the command only prints JSON.
...
Use `lastUpdated` to control the date shown at the bottom of each docs page. The optional `label` field lets you replace the default `"Last updated"` prefix with any string — useful for localization or rebranding.
...
| `position` | `"footer" \| "below-title"` | `"footer"`       | Render beside "Edit on GitHub" or below the page title   |

Set `label` to an empty string to display only the formatted date with no prefix text.
...
For code-heavy guides where examples make up a large part of the page, set `includeCode: true` so fenced and inline code blocks are counted toward the estimate:
...
- the estimate uses `220` words per minute unless you override it
- labels render as `5 min read` by default; set `format: "short"` to render `5 min`
- fenced code blocks, inline code, links, images, and raw HTML are stripped before counting words

- set `includeCode: true` in `ReadingTimeConfig` to count code block and inline code words toward the estimate instead of stripping them
- changelog pages do not render the reading-time label
- page frontmatter wins when `readingTime` is present, even if the global config is disabled
...
- global `readingTime.enabled: true` -> pages get a computed label by default
- page `readingTime: false` -> hide the label for that page
- page `readingTime: true` -> force a computed label for that page
- page `readingTime: 12` -> force an exact `12 min read` label for that page

The global `format` setting applies to computed labels and exact per-page values. With
`format: "short"`, the same `readingTime: 12` override renders as `12 min`.

<Callout type="info" title="Placement with page actions">
...
Use `openDocs: true` for the built-in ChatGPT + Claude provider list, or pass provider strings and `target: "markdown"` to send tools directly to the public `.md` route for the page.

For deeper customization, use provider objects. This keeps the built-in preset but opens Cursor through the app deeplink:
