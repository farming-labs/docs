...
without a model, network request, or arbitrary command execution. Each task runs through the docs
search and context pipeline, then checks retrieval recall and rank, canonical citations,
framework/version selection, executable example metadata, and exact UTF-8 context usage.
...
section URLs. The context budget uses a hard UTF-8 byte ceiling and also reports estimated tokens
plus a conservative one-token-per-byte upper bound.
