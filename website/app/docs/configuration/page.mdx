...
<Tabs items={["Next.js", "SvelteKit"]}>
  ...
  <Tab value="SvelteKit">
    ...
    > SvelteKit requires `contentDir` (path to your markdown files) and `nav` (sidebar title/URL) since routing is handled differently from Next.js.
  </Tab>
</Tabs>
...
| `contentDir` | `string` | same as `entry` | Path to content files (SvelteKit only) |
...
Controls the light/dark mode switcher. Works on both Next.js and SvelteKit.
