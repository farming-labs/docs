...
```text title="internationalization.txt"
docs/
  en/
    page.md
...
**Next.js / TanStack Start / SvelteKit / Nuxt:** `feedback.onFeedback` is called from the built-in page footer UI.

**Astro:** the built-in UI still comes from `docs.config`, and the same payload is emitted through `window.__fdOnFeedback__` and the `fd:feedback` custom event for client-side analytics hooks.
...
<Tabs items={["Next.js", "TanStack Start", "SvelteKit", "Astro", "Nuxt"]}>
  ...
  <Tab value="TanStack Start">
    ```ts title="tanstack-start-ask-ai.ts"
    ai: {
      enabled: true,
      mode: "floating",
      floatingStyle: "full-modal",
