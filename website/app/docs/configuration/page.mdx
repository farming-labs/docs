...
`sidebar.folderIndexBehavior: "toggle"` if you want the parent row to only open and close its
children instead of navigating.
...
<Callout type="info" title="Markdown via content negotiation">
  ...
  that send `Accept: text/markdown` receive the same machine-readable output as
  `/docs/installation.md`. Other adapters should use the `.md` URL or the API format route.
</Callout>
