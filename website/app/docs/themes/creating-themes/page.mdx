...
The `theme` and `darkTheme` fields drive syntax highlighting during the Next.js build. Pass the live theme object to `withDocs()` so these values are picked up:
...
When the `theme` field is omitted, `withDocs()` reads `docs.config` directly to detect whether the shadcn preset is in use and selects `github-light-default` / `vesper` accordingly; all other configs fall back to `github-light` / `github-dark`.
