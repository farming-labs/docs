...
| `pnpm exec docs deploy` | You want to sync, validate the key, and request a preview |
| `pnpm exec docs cloud deploy` | You prefer the explicit Cloud namespace |
| `pnpm exec docs deploy --json` | CI needs machine-readable output |
| `pnpm exec docs deploy --config <path>` | The docs config file is outside the project root |
