...
1. Detect your framework (Next.js, SvelteKit, Astro, or Nuxt), or ask you to pick one
...
3. Ask for the docs entry path (default: `docs` — press **Enter** to accept)
4. Ask about path aliases and which global CSS file to use (defaults are offered; press **Enter** to accept)
5. Generate config, layout, CSS, and sample pages
6. Install dependencies and optionally start the dev server
...
To **upgrade** all docs packages to the latest later, run from your project root: `npx @farming-labs/docs@latest upgrade` (framework is auto-detected). See [CLI — Upgrade](/docs/cli#upgrade) for options.
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npx @farming-labs/docs@latest init --template next --name my-docs     # Next.js
    npx @farming-labs/docs@latest init --template nuxt --name my-docs     # Nuxt
    npx @farming-labs/docs@latest init --template sveltekit --name my-docs # SvelteKit
    npx @farming-labs/docs@latest init --template astro --name my-docs    # Astro
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm dlx @farming-labs/docs@latest init --template next --name my-docs     # Next.js
    pnpm dlx @farming-labs/docs@latest init --template nuxt --name my-docs     # Nuxt
    pnpm dlx @farming-labs/docs@latest init --template sveltekit --name my-docs # SvelteKit
    pnpm dlx @farming-labs/docs@latest init --template astro --name my-docs    # Astro
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn dlx @farming-labs/docs@latest init --template next --name my-docs     # Next.js
    yarn dlx @farming-labs/docs@latest init --template nuxt --name my-docs     # Nuxt
    yarn dlx @farming-labs/docs@latest init --template sveltekit --name my-docs # SvelteKit
    yarn dlx @farming-labs/docs@latest init --template astro --name my-docs    # Astro
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bunx @farming-labs/docs@latest init --template next --name my-docs     # Next.js
    bunx @farming-labs/docs@latest init --template nuxt --name my-docs     # Nuxt
    bunx @farming-labs/docs@latest init --template sveltekit --name my-docs # SvelteKit
    bunx @farming-labs/docs@latest init --template astro --name my-docs    # Astro
    ```
  </Tab>
</Tabs>
