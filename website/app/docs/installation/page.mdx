...
<Tabs items={["Next.js", "TanStack Start", "SvelteKit", "Astro", "Nuxt"]}>
  ...
  <Tab value="TanStack Start">
    ...
    ### 4. Create `src/lib/docs.functions.ts`
    ...
    ### 6. Import the theme CSS in `src/styles/app.css`
    ...
    export const ServerRoute = createServerFileRoute("/api/docs").methods({
      GET: ({ request }) => docsServer.GET(request),
      POST: ({ request }) => docsServer.POST(request),
    });
    ```
