...
The contract is optional and additive. Valid contracts appear in the served Markdown frontmatter and
as a deterministic `## Agent Contract` section, and MCP page summaries/read responses expose the
same normalized data. Malformed fields are ignored while serving the page and reported by
`docs review`, so an authoring mistake does not take the docs endpoint down.
