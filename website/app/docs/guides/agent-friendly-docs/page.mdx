...
<Callout type="tip" title="Keep Agent blocks additive">
  Treat `<Agent>` as the place for implementation hints, verification steps, and route-specific
  behavior that would feel noisy in the visible article. Do not duplicate the whole page there.
</Callout>
