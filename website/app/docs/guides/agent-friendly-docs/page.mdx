...
- `/.well-known/agent-skills/index.json` publishes the site skill with a SHA-256 digest, and
  `/.well-known/agent-skills/<name>/SKILL.md` returns the exact hashed bytes
...
standards routes and HTTP `Link` headers are additive and cross-link back to it.
