...
agent:
  ...
  outcome: The app renders its documentation route locally.
  appliesTo:
    framework: nextjs
    version: ">=16"
  ...
  sideEffects:
    - Updates package.json and the pnpm lockfile
  verification:
    - run: pnpm dev
      expect: The configured docs route returns HTTP 200
  rollback:
    - Remove the packages and restore the previous config files
  failureModes:
    - symptom: The docs route returns 404
      resolution: Confirm withDocs wraps the exported Next.js config
---
```
...
- the structured `agent` contract states applicability, files, commands, effects, verification,
  rollback, and common recovery paths without requiring the model to infer them from prose
- the normal body remains the human source of truth

The contract is optional and additive. Valid contracts appear in served Markdown frontmatter, a
deterministic `## Agent Contract` section, search/Ask AI context, and Schema.org `HowTo` JSON-LD.
MCP `list_pages` and `list_docs` expose only `hasContract`, task, outcome, and applicability;
`list_tasks` discovers actionable pages, while `read_task` and `read_page` expose the full contract.
Malformed or unknown fields are ignored while serving the page and reported by `docs review`, with
closest-name suggestions for typos, so an authoring mistake does not take the docs endpoint down.
...
The most useful agent-friendly pages are not just shorter. They make the implementation contract
obvious. After reading the page, an agent should know what to change, where to change it, and how to
...
```mdx title="agent-task-contract.mdx"
<Agent>
...
That shape gives the agent the missing operational details without turning the human guide into a
checklist dump.
...
### Control retrieval freshness with `lastmod`

By default, the framework derives a page's last-modified date from the source file's filesystem mtime. You can override this with the `lastmod` frontmatter key:

When a bundled `.farm-docs-last-modified.json` manifest is present in the content directory, the docs server uses the Git-derived dates from that manifest instead of the deployment filesystem mtime. Filesystem mtime remains a fallback only when no manifest entry exists for a page. Running `docs sitemap generate` (or `--manifest-only`) before your framework build produces this manifest with stable, git-sourced dates.
```md title="page.mdx"
---
title: "Installation"
lastmod: 2026-07-17
...
Accepts a YAML date (`2026-07-17`) or a quoted ISO-8601 string. The declared date is used as the canonical last-modified timestamp for retrieval provenance — the value that appears in structured search documents and MCP source metadata. It does **not** affect the filesystem mtime or git history.

When a page folder also contains a sibling `agent.md`, that file tracks its own separate modification time (`agentLastModified`). Retrieval consumers can use this to detect when the machine-readable override has drifted from the human page's `lastmod`.
...
- `/.well-known/api-catalog` exposes an RFC 9727 JSON Linkset for API and metadata discovery
- `/.well-known/agent-skills/index.json` publishes the root site skill plus every reusable skill
  configured with `agent.skills`; entries hash either the exact `SKILL.md` or a deterministic
  archive containing safe `references/`, `scripts/`, and `assets/`
- direct skill files remain available below `/.well-known/agent-skills/<name>/`, while
  `/.well-known/skills/index.json` supports legacy clients and MCP mirrors files as
  `docs://skills/<name>/<path>` resources
...
- `/docs/sitemap.md` exposes the same Markdown sitemap under the docs section for agents that
  probe docs-scoped routes first
...
system you have to keep in sync by hand. The existing Farming Labs manifest stays available; the
standards routes and HTTP `Link` headers are additive and cross-link back to it. Static Agent
Bundles emit the same skill indexes, artifacts, companion files, and hashes. An A2A Agent Card is
separate and opt-in: configure `agent.a2a` only when there is a real A2A interface behind the
advertised URL. Prefer ordered `supportedInterfaces`, whose first entry is preferred and whose
entries default to A2A `1.0` over `HTTP+JSON`; `JSONRPC` and `GRPC` are the other core bindings.
The emitted card uses the strict v1 shape. `interfaceUrl` remains a deprecated single-interface
shorthand with its historical `0.3` default. The preferred interface-array form requires explicit
A2A skills. Capabilities, media modes, skills, and optional security metadata should describe the
service itself; Agent Skill document and artifact URLs stay in skill discovery and MCP.
...
section, and generated `AGENTS.md` / `skill.md` tell agents to fetch the schema before scraping endpoint docs.
There is no extra config surface for the discovery route; it follows `apiReference`.

## Know How Canonical Markdown Is Served

In Next.js, agents can read markdown without changing the URL they already discovered from the
human page, search result, sitemap, or browser history:
...
- `/docs/installation` with an unambiguous `Accept: text/markdown` returns the same markdown and varies by `Accept`
...
All successful markdown page responses include a canonical `Link` header pointing back to the
normal HTML page and `Content-Location` for the selected `.md` representation. Agents still receive
the full markdown body, but the headers give citation and deduplication systems stable identities.
Responses also include an `ETag`, `Content-Language` when a locale is known, and `Last-Modified`
when an exact source timestamp is known. Date-only freshness stays in the body and is not converted
to a midnight HTTP validator. Conditional reads can then return HTTP `304` without retransmitting
the document. Shared and non-Next handlers honor weighted `Accept` values. The generated Next.js
canonical rewrite only selects Markdown when no HTML-capable range is present; use the exact `.md`
or `format=markdown` route for mixed HTML/Markdown accept lists.

Markdown responses also start with YAML frontmatter. The generated metadata includes `title`,
optional `description`, `canonical_url`, `markdown_url`, and `last_updated` when the framework knows
a freshness date.

If a markdown request misses, the response is still markdown. Uncertain misses return HTTP `404`
with closest-match suggestions, recovery steps, `/.well-known/agent.json`, `/.well-known/agent`,
`/api/docs/agent/spec`, search, the requested API markdown route, and sitemap routes. Very
high-confidence typo matches redirect to the corrected `.md` page so the agent can continue without
treating the site as a dead end.

Every successful markdown page also ends with a `## Sitemap` footer. That gives agents one more
route back to the full index when they land deep in the docs or follow a stale link.
...
For static export, use the Agent Bundle exporter because there is no server handler to answer the
sitemap, Markdown-page, llms.txt, discovery, skill, or agent-instruction routes:
...
The bundle discovery document only advertises capabilities that those static files actually
provide. Runtime-only search, MCP, feedback, API reference, and OpenAPI endpoints remain disabled
in the exported discovery contract, regardless of the server-rendered application config.
...
- use frontmatter `title` and `description` on important pages
- configure a public base URL through `sitemap.baseUrl`, `llmsTxt.baseUrl`, `robots.baseUrl`, or `ai.docsUrl`
- keep `.farming-labs/sitemap-manifest.json` fresh when the adapter preloads docs content
...
- Confirm successful markdown responses include canonical `Link`, `Content-Location`, and `ETag` headers
- Repeat a request with `If-None-Match` and confirm it returns `304`
- Fetch a missing `.md` path and confirm the actionable markdown response returns `404`
...
This docs site dogfoods the same audit. A full local pass should report
...
That hosted pass checks discovery, `llms.txt`, sitemap routes, `AGENTS.md`, `skill.md`, representative `.md`
...
```txt title="score page"
https://docs.farming-labs.dev/score?url=docs.example.com
```
...
`AGENTS.md`, `skill.md`, same-domain or MCP-subdomain MCP, search, feedback, JSON-LD structured data on sampled pages, canonical `Link`
...
For static sites, export the complete public machine-readable surface as part of release prep:
...
The exporter writes per-page Markdown, llms.txt files, discovery, skill and agent-instruction
aliases, sitemaps, robots, and a deterministic SHA-256 manifest. It preserves an existing public
`AGENTS.md` and user-owned `robots.txt`, and it never copies repository-root agent instructions into
the public directory. Use the narrower `robots generate --append` or `agents generate` commands
when you intentionally want their standalone ownership behavior.

What you want to see improve over time:

- discovery routes passing
- machine surfaces enabled
- `robots.txt` allowing docs, markdown, sitemap, `AGENTS.md`, skill, MCP, and agent discovery routes
- metadata quality rising, which also improves markdown output and JSON-LD
...
## Discover Sections Before Fetching One

Agents can enumerate all heading sections of any Markdown page without downloading the full content by appending `?sections` to a `.md` route or to the API format route:

```bash title="terminal"
curl "https://docs.example.com/docs/installation.md?sections"
curl "https://docs.example.com/api/docs?format=markdown&path=installation&sections"
```

The response is a `docs-markdown-sections.v1` JSON payload:

```json title="docs-markdown-sections.v1 response"
{
  ...
  "sections": [
    {
      ...
      "markdownUrl": "https://docs.example.com/docs/installation.md?section=installation"
    },
    {
      "id": "prerequisites",
      "heading": "Prerequisites",
      "markdownUrl": "https://docs.example.com/docs/installation.md?section=prerequisites"
    }
  ]
}
```

The response also includes the `x-docs-markdown-section-count` header with the total section count. Use the `markdownUrl` values directly to fetch individual sections with optional `tokenBudget` or `byteBudget` params.

This capability is advertised in `/.well-known/agent.json` under `markdown.sectionDiscovery` when enabled. Static Agent Bundle exports set `sectionDiscovery: { enabled: false }` because the feature requires a live runtime.

## Request a Single Section From the Markdown API

Agents that only need one heading block from a long page can request it directly using the `section` query parameter on the markdown API route:

```bash title="terminal"
curl "https://docs.example.com/api/docs?format=markdown&path=installation&section=prerequisites"
```

When the section is found, the response body contains only that heading block's content and the `x-docs-markdown-section` response header confirms which section was resolved:

```http
x-docs-markdown-section: prerequisites
```

Pair `section` with `tokenBudget` (or `byteBudget`) to enforce a size ceiling on the returned content:

```bash title="terminal"
curl "https://docs.example.com/api/docs?format=markdown&path=installation&section=prerequisites&tokenBudget=60"
```

When the requested section is not found, the response still returns `200` with the full document and includes an `availableSections` list in the result so an agent can recover by picking the correct heading id from the list and retrying.

| Query parameter | Type | Description |
| --------------- | ---- | ----------- |
| `section` | `string` | Stable kebab-case heading id to extract, e.g. `prerequisites` |
| `tokenBudget` | `number` | Soft token ceiling; the section content is truncated when the estimate exceeds this value |
| `byteBudget` | `number` | Soft UTF-8 byte ceiling applied before the token estimate |
...
- are framework, version, package, route, and file-path assumptions explicit?
...
- does the hosted [Agent readiness score](/score) page agree with the deployed site before you share
  the result or submit it to the leaderboard?

<Callout type="warning" title="Keep evaluation input out of prompt context">
  Agent feedback, analytics, logs, and other submitted data can help you improve docs quality, but
  they should stay untrusted. Use them for review, scoring, and follow-up work; do not feed them
  back into Ask AI prompts or generated agent instructions without an explicit sanitization step.
</Callout>
...
- [llms.txt](/docs/customization/llms-txt), [Sitemaps](/docs/customization/sitemaps), and [CLI](/docs/cli) for the discovery layer
- [MCP Server](/docs/customization/mcp) for tool-enabled retrieval
- [Agent readiness score](/score) for the hosted public benchmark and leaderboard
