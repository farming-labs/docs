...
when freshness is known. Conditional reads can then return HTTP `304` without retransmitting the
document. When `Accept` lists both HTML and markdown, the shared handler honors their `q` values.
