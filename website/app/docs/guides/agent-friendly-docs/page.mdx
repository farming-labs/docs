...
pages, `robots.txt`, JSON-LD structured data, markdown alternate head links, and MCP at both
...
feedback, JSON-LD structured data on sampled pages, and the
`<link rel="alternate" type="text/markdown">` head links that point agents to each page's `.md`
route. Existing leaderboard entries hydrate from the saved report, so a shared score URL can be
reviewed without triggering a new calculation unless no saved result exists.
...
- `robots.txt` allowing docs, markdown, sitemap, skill, MCP, and agent discovery routes
