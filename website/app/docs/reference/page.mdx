...
| `sectionDiscovery` | `boolean` | Whether runtime Markdown routes advertise and serve the `?sections` index endpoint. Defaults to `true`; set `false` to suppress the endpoint and the `markdownSectionDiscovery` capability flag in `/.well-known/agent.json`. Static Agent Bundle exports always set this to `false`. |
  tokenBudget: 1800
