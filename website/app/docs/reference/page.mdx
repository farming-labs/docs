...
`components` is merged into the default MDX component map, so you can both add your own components and replace built-ins such as `Callout`, `Tabs`, or `HoverLink`. For usage examples and a live demo, see [Components](/docs/customization/components).
...
Generates API reference pages from framework route conventions.
