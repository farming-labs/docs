...
| Property | Type | Required | Description |
| -------- | ---- | -------- | ----------- |
| `interfaceUrl` | `string` | yes | HTTP(S) URL of an implemented A2A interface |
| `name` | `string` | yes | Agent display name |
| `description` | `string` | yes | Agent purpose |
| `documentationUrl` | `string` | yes | HTTP(S) documentation URL for the A2A service |
| `provider` | `{ organization: string; url: string }` | yes | Provider identity and HTTP(S) URL |
| `version` | `string` | no | Agent version; defaults to `"1.0.0"` |
| `protocolVersion` | `string` | no | Implemented A2A protocol version; defaults to `"0.3"` |
| `protocolBinding` | `string` | no | Implemented A2A transport binding; defaults to `"HTTP+JSON"` |

When `a2a` is set, `/.well-known/agent-card.json` advertises the configured interface and published
skills, using A2A protocol `0.3` over `HTTP+JSON` by default. When it is omitted, the route is not
published. Do not point `interfaceUrl` at a docs, Ask AI, or MCP route; those transports do not
implement A2A.
