...
| `includeTitle` | `boolean` | `false`      | Prepend the page title to copied content — as `# Title` for `"markdown"` format or as a plain text prefix for `"text"` format; no-op if the title already appears as the first line |
