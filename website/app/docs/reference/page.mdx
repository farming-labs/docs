...
<Callout type="info" title="Renderer support">
  `renderer` defaults to `"scalar"`. Set `renderer: "fumadocs"` in **Next.js** if you want the
...
| `renderer`  | `"scalar" \| "fumadocs"` | `"scalar"` | API reference UI renderer. `fumadocs` is currently supported in Next.js only |
