...
| `contentDir` | `string` | same as `entry` | Path to content files. SvelteKit only — Next.js uses `app/{entry}/` |
...
Sidebar visibility and customization.

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `enabled` | `boolean` | `true` | Show or hide the sidebar |
| `component` | `Component` | — | Custom sidebar component (Next.js only) |
...
| `triggerComponent` | `ReactNode` | — | Custom floating button (Next.js only) |
...
| `loadingComponent` | `(props: { name: string }) => ReactNode` | — | Custom loading indicator (Next.js only) |
...
Wraps your Next.js config with docs framework support. Handles MDX, routing, and search index generation.
...
Creates the docs layout component from your config.
