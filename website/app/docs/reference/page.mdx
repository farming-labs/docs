...
agent:
  ...
  commands:
    - run: pnpm exec docs doctor --agent
      description: Resolve config and compare discovery metadata with the public schema.
  verification:
    - run: pnpm exec docs doctor --agent
      expect: Config loading confidence and discovery/config/schema consistency pass after the option change.
    - description: Query MCP get_config_schema for the changed option.
      expect: The returned type, default, and description match the option used in docs.config.
  rollback:
    - Restore the previous option value and any route wiring or environment variables added with it.
  failureModes:
    - symptom: TypeScript rejects a documented option.
      resolution: Confirm all Farming Labs packages use the same version and use docs.config.tsx when JSX is present.
    - symptom: The config accepts an option but discovery advertises a different capability state.
      resolution: Run the agent doctor and update stale adapter or static-export artifacts from the resolved config.
---
...
Runtime shape that describes whether the local MCP server should be treated as the search target. Exported from `@farming-labs/docs` and `@farming-labs/docs/server`.
...
| `tools` | `{ searchDocs?: boolean }` | Tool-level toggle; `searchDocs` controls whether `search_docs` is exposed |

### `DocsLocalMcpSearchRuntimeInput`

Accepted input shape for `resolveLocalDocsMcpSearchConfig`. A partial form of `DocsLocalMcpSearchRuntimeConfig` — all fields are optional. Exported from `@farming-labs/docs` and `@farming-labs/docs/server`.

### `DocsSearchRequestResolutionOptions`

Options accepted by the third parameter of `resolveSearchRequestConfig`. Exported from `@farming-labs/docs` and `@farming-labs/docs/server`.

| Property | Type | Description |
| -------- | ---- | ----------- |
| `localMcp` | `DocsLocalMcpSearchRuntimeInput \| null \| undefined` | When present, `resolveSearchRequestConfig` calls `resolveLocalDocsMcpSearchConfig` before the self-referential URL check to detect whether the MCP endpoint resolves to the local server |

### `resolveLocalDocsMcpSearchConfig`

Public function exported from `@farming-labs/docs` and `@farming-labs/docs/server`. Given a `McpDocsSearchConfig`, an optional `DocsLocalMcpSearchRuntimeInput`, and an optional request URL, returns a modified search config that uses direct simple search when the configured MCP endpoint resolves to the local server. Returns the original config unchanged for external endpoints.
...
`resolveSearchRequestConfig` now accepts an optional third `options: DocsSearchRequestResolutionOptions` argument. Pass `{ localMcp: config.mcp }` to enable local-MCP detection in custom server adapters.
...
| Property  | Type                                  | Default        | Description |
| --------- | ------------------------------------- | -------------- | ----------- |
| `enabled` | `boolean`                             | `true`         | Show or hide the theme switcher |
| `default` | `"light" \| "dark" \| "system"`       | `"system"`     | When `enabled` is `false`, this value is applied as a **forced** theme (hiding the switcher). When the toggle is enabled, it is used as the **initial** theme that visitors can still change. `"system"` skips forced and default derivation in both cases. |
| `mode`    | `"light-dark" \| "light-dark-system"` | `"light-dark"` | Toggle mode presented to the user |
| `og`          | `OGConfig`                                      | —                | Open Graph image config                                                              |

---

## `DocsMarkdownSection`, `DocsMarkdownSectionRequest`, and `DocsMarkdownSectionResult`

Three interfaces that power section-addressable markdown reads. All are exported from `@farming-labs/docs`.

### `DocsMarkdownSection`

Represents a single heading-delimited section parsed from a markdown document.

| Property | Type | Description |
| -------- | ---- | ----------- |
| `id` | `string` | Stable kebab-case heading id used as the `section` query parameter |
| `heading` | `string` | Raw heading text |
| `level` | `number` | Heading level (1–6) |
| `content` | `string` | Full markdown content of the section, including the heading line |
| `startLine` | `number` | 1-based line number of the heading that opens this section, relative to the Markdown body |
| `endLine` | `number` | 1-based inclusive line number of the last line in this section |
| `startLine` | `number` | Zero-based line index where the section starts |
| `endLine` | `number` | Zero-based line index where the section ends (exclusive) |

### `DocsMarkdownSectionRequest`

Input parameters for a section-addressable markdown request.

| Property | Type | Description |
| -------- | ---- | ----------- |
| `section` | `string \| null` | Heading id to select, e.g. `"prerequisites"`. When omitted the full document is returned. |
| `tokenBudget` | `number \| null` | Soft token ceiling; the response is truncated when the estimated token count exceeds this value |
| `byteBudget` | `number \| null` | Soft UTF-8 byte ceiling applied before the token estimate |

### `DocsMarkdownSectionResult`

Result returned by `resolveDocsMarkdownSectionRequest`.

| Property | Type | Description |
| -------- | ---- | ----------- |
| `found` | `boolean` | `true` when the requested section was resolved |
| `requestedSection` | `string` | The `section` value from the request, when provided |
| `section` | `DocsMarkdownSection` | The resolved section, present when `found` is `true` |
| `document` | `string` | The content to return — the matched section body or the full document |
| `truncated` | `boolean` | `true` when the document was cut to fit the budget |
| `tokenBudget` | `number` | Applied token budget |
| `byteBudget` | `number` | Applied byte budget |
| `estimatedTokens` | `number` | Estimated token count for the returned `document` |
| `utf8Bytes` | `number` | UTF-8 byte length of the returned `document` |
| `availableSections` | `DocsMarkdownSection[]` | All sections parsed from the full document; useful for agent recovery when `found` is `false` |

### Related functions

Three functions are exported from `@farming-labs/docs` for working with these types:

| Function | Description |
| -------- | ----------- |
| `collectDocsMarkdownSections(markdown)` | Parse a markdown string into an array of `DocsMarkdownSection` objects, one per heading block |
| `selectDocsMarkdownSection(sections, id)` | Find a section by its stable heading id; returns the matching `DocsMarkdownSection` or `undefined` |
| `resolveDocsMarkdownSectionRequest(markdown, request)` | Combine parsing, section lookup, and budget enforcement in one call; returns a `DocsMarkdownSectionResult` |

The constant `DOCS_MARKDOWN_SECTION_MAX_BUDGET = 1_000_000` is the hard upper limit accepted for `tokenBudget` and `byteBudget`.

### `DocsMcpPageSectionIndex`

`DocsMcpPageSectionIndex` is the body-free section discovery metadata returned by the `list_page_sections` MCP tool. It is exported from `@farming-labs/docs` as a type alias for `DocsMarkdownSectionIndex`.

### `DocsMarkdownSectionIndex`

The `docs-markdown-sections.v1` JSON payload returned by the `?sections` endpoint.

| Property | Type | Description |
| -------- | ---- | ----------- |
| `format` | `"docs-markdown-sections.v1"` | Fixed format identifier |
| `sectionCount` | `number` | Total number of sections in the document |
| `sections` | `DocsMarkdownSectionMetadata[]` | Ordered array of section descriptors |

### `DocsMarkdownSectionMetadata`

One entry in the `sections` array of a `DocsMarkdownSectionIndex`.

| Property | Type | Description |
| -------- | ---- | ----------- |
| `id` | `string` | Stable kebab-case heading id, usable as the `section` query parameter |
| `heading` | `string` | Raw heading label |
| `level` | `number` | Heading level (1–6) |
| `parentId` | `string \| null` | Id of the nearest ancestor heading, or `null` for top-level sections |
| `startLine` | `number` | 1-based line of the heading within the Markdown body |
| `endLine` | `number` | 1-based inclusive final line of the section |
| `estimatedTokens` | `number` | Conservative token estimate for the section content |
| `markdownUrl` | `string` | Absolute URL for fetching this section with `?section={id}` |

### `DocsMarkdownSectionIndexOptions`

Options accepted by `buildDocsMarkdownSectionIndex`.

| Property | Type | Description |
| -------- | ---- | ----------- |
| `markdown` | `string` | Raw Markdown source to index |
| `baseUrl` | `string` | Base URL used to construct `markdownUrl` values for each section |

### Section-index functions

Two additional functions are exported from `@farming-labs/docs`:

| Function | Description |
| -------- | ----------- |
| `buildDocsMarkdownSectionIndex(options)` | Parse a Markdown string and build a `DocsMarkdownSectionIndex` response object with per-section metadata and absolute `markdownUrl` values |
| `isDocsMarkdownSectionIndexRequest(request)` | Return `true` when the incoming `Request` includes the `sections` query parameter and should receive a section-index response instead of the full Markdown body |

The constant `DOCS_MARKDOWN_SECTION_INDEX_FORMAT = "docs-markdown-sections.v1"` is the fixed format string included in every index response and advertised in `/.well-known/agent.json`.
## `DocsConfigMap`, `DocsConfigMapPointer`, and config-map constants

The config-map endpoint (`GET /api/docs?format=config`) returns a `DocsConfigMap` JSON document
built from the active docs config. All five framework server adapters handle this request
automatically.
...
| -------- | ----- | ----------- |
| `DEFAULT_DOCS_CONFIG_FORMAT` | `"docs-config-map.v1"` | Schema version identifier included in the response and in the agent discovery spec |
| `DEFAULT_DOCS_CONFIG_ROUTE` | `"/api/docs?format=config"` | Canonical route for the config-map endpoint |

### `DocsConfigMap`

The JSON response shape. Contains a `format` field set to `"docs-config-map.v1"` and a `pointers`
array of `DocsConfigMapPointer` entries. Sensitive config values are redacted before serialization.

### `DocsConfigMapPointer`

| Property | Type | Description |
| -------- | ---- | ----------- |
| `path` | `string` | Dot-path to the config field, e.g. `"mcp.tools.readPage"` |
| `kind` | `string` | Coarse value kind at this pointer, e.g. `"boolean"`, `"string"`, `"object"` |
...
The agent discovery spec (`GET /.well-known/agent.json`) now includes:
...
## Farm.js adapter — `withDocs` Vite config

When using the `@farming-labs/farmjs` adapter, `withDocs()` accepts an optional `vite` field on the config object passed to it.

| Option | Type | Description |
| ------ | ---- | ----------- |
| `vite` | `FarmViteConfigLike \| ((config: FarmViteConfigLike) => FarmViteConfigLike)` | Optional Vite config object or factory function. The adapter injects its MDX/remark transform plugin ahead of any plugins you supply here. |

Both object and function forms are supported:

```ts title="farm.config.ts"
import { withDocs } from "@farming-labs/farmjs";
...
export default withDocs({
  // factory form — receives the resolved config and returns a merged object
  vite: (config) => ({ ...config, plugins: [myPlugin()] }),
});
```

In both cases the adapter-owned MDX transform is prepended before any plugins you provide, so MDX processing always runs first.
...
Built-in product and usage event stream. Use it for page views, search activity, Ask AI request
counts, feedback, page actions, machine-readable markdown reads, agent feedback routes, and MCP
usage. See [Analytics](/docs/customization/analytics) for the full guide and event list.
...
Built-in trace event stream for agent runtime debugging. Use it for Ask AI and MCP spans, timing,
status, previews, errors, retries, and final handoffs. See
[Observability](/docs/customization/observability) for the full guide and trace event list.
...
<Callout type="info" title="Separate from analytics">
...
| `security` | `DocsMcpSecurityConfig`  | see below         | HTTP Origin validation, opt-in authentication, and request limits |
...
Authentication is opt-in. Omitting `authenticate` keeps HTTP MCP public, while Origin validation
and the request-size limit still apply. The stdio transport is unaffected.

| Property            | Type                                            | Default         | Description |
| ------------------- | ----------------------------------------------- | --------------- | ----------- |
| `allowedOrigins`    | `"same-origin" \| string[] \| callback`         | `"same-origin"` | Policy for a supplied HTTP Origin header; requests without Origin are accepted |
| `authenticate`      | `(context) => principal \| null \| Response`     | public          | Optional Web Request callback; adding it makes authentication required |
| `protectedResource` | `DocsMcpProtectedResourceConfig`                | —               | Opt-in RFC 9728 metadata, Bearer challenges, and endpoint-wide scope requirements; active only with `authenticate` |
| `maxBodyBytes`      | `number`                                        | `1048576`       | Maximum Streamable HTTP POST body size |
| `cors`              | `boolean \| DocsMcpCorsConfig`                   | `true`          | Exact-Origin browser CORS for accepted Origins; set false to suppress headers |
...
export default defineDocs({
  mcp: {
    security: {
      ...
      authenticate: async ({ request, pathname, resource }) => {
        const user = await authenticateRequest(request, { audience: resource });
        return user
          ? { id: user.id, scopes: user.scopes, claims: { pathname, resource } }
          : null;
      },
    },
  },
...
POST bodies are capped before `allowedOrigins` or `authenticate` callbacks run. Browser preflight
uses the `OPTIONS` handler and does not run authentication.

### `DocsMcpProtectedResourceConfig`

RFC 9728 protected-resource discovery for OAuth-aware HTTP MCP clients. This object only becomes
active when `authenticate` is also configured. Omitting both keeps HTTP MCP public by default.
It does not issue or validate tokens: `authenticate` remains responsible for signature, issuer,
expiry, the exact callback `resource` audience, and token-derived scope validation. The callback
`request` retains the real incoming URL and query string.

| Property                | Type                | Required | Description |
| ----------------------- | ------------------- | -------- | ----------- |
| `authorizationServers`  | `readonly string[]` | yes      | Non-empty list of HTTPS OAuth issuer URLs without query or fragment; loopback HTTP is accepted for development |
| `scopesSupported`       | `readonly string[]` | no       | Scopes advertised as `scopes_supported` |
| `requiredScopes`        | `readonly string[]` | no       | Scopes every principal returned by `authenticate` must include |
| `resourceName`          | `string`            | no       | Human-readable protected-resource name; defaults to the resolved MCP server name |
| `resourceDocumentation` | `string`            | no       | Absolute HTTP(S) URL with human-readable authentication instructions |

The metadata routes are public and support `GET` and `HEAD`:

- `/.well-known/oauth-protected-resource/mcp` for `/mcp`
- `/.well-known/oauth-protected-resource/.well-known/mcp` for `/.well-known/mcp`
- `/.well-known/oauth-protected-resource/api/docs/mcp` for the default canonical route; a custom
  `mcp.route` produces the matching path instead

When `authenticate` returns `null`, protected-resource mode returns `401` with a Bearer
`WWW-Authenticate` challenge containing the metadata URL and required scopes. A valid principal
missing a required scope receives `403` with `error="insufficient_scope"`. Returning a Web
`Response` bypasses framework-generated challenges and preserves the custom response. Use HTTPS for
hosted MCP and authorization-server URLs; plain HTTP is appropriate only for loopback development.
Each alias is a distinct OAuth resource identifier, so token audience validation must accept the
exact endpoint used by the client.
For Next.js, protected MCP is incompatible with `basePath`; host it at the origin root or terminate
the MCP and RFC 9728 routes at an edge proxy. Inline MCP objects are detected automatically. Pass
the live docs config to `withDocs(nextConfig, docsConfig)` when MCP configuration is imported,
spread, or shorthand.
...
| Property           | Type       | Default | Description |
| ------------------ | ---------- | ------- | ----------- |
| `allowedHeaders`   | `string[]` | MCP standard headers | Additional browser request headers allowed during preflight |
| `exposedHeaders`   | `string[]` | MCP response headers | Additional response headers exposed to browser JavaScript |
| `allowCredentials` | `boolean`  | `false` | Emit credentialed CORS for the validated exact Origin; wildcard credentials are never emitted |
| `maxAgeSeconds`    | `number`   | `600`   | Browser preflight cache lifetime |
The default `allowedHeaders` set includes `Authorization`, `Content-Type`, `Last-Event-ID`, `MCP-Method`, `MCP-Name`, `MCP-Protocol-Version`, and `MCP-Session-Id`. Use `cors.allowedHeaders` to add application-specific headers on top of these defaults.
...
| `listPageSections`  | `boolean` | Expose the `list_page_sections` tool that returns body-free section discovery metadata for a page (default: `true`) |
| `listTasks`         | `boolean` | Expose the `list_tasks` tool for concise actionable-task discovery |
| `readTask`          | `boolean` | Expose the `read_task` tool for full structured contracts |
| `getNavigation`     | `boolean` | Expose the `get_navigation` tool |
| `searchFacets`      | `boolean` | Expose the `list_search_facets` tool for body-free framework, version, package, and tag discovery (default: `true`) |
| `listContentChanges` | `boolean` | Expose the `list_content_changes` tool, which returns body-free document change metadata with polling support via `DOCS_MCP_CONTENT_CHANGES_CURRENT_URI` and `DOCS_MCP_CONTENT_CHANGES_URI_TEMPLATE` resources (default: `false`) |
| `hydrateContentChanges` | `boolean` | Expose `hydrate_content_changes` for budget-aware section bodies and deletion tombstones after polling `list_content_changes`. Accepts a `tokenBudget` parameter (default `5000`; min `4`; max `32000`) (default: `true`) |
...
| `getContext`        | `boolean` | Expose deterministic `get_context` retrieval with a conservative UTF-8 byte ceiling |
...
`list_pages` and `list_docs` keep their `agent` value concise: `hasContract`, `task`, `outcome`, and
`appliesTo` only. Use `list_tasks` to filter actionable pages by query/framework/version/package,
then `read_task` for the full normalized contract (also available as Markdown through `read_page`).

Framework notes:

- **Next.js:** `withDocs()` auto-generates the default `/api/docs/mcp` route and public `/mcp` plus `/.well-known/mcp` rewrites

### `FarmDocsAdapterOptions`

Options accepted by the second argument of `withDocs(farmConfig, options)` in `@farming-labs/farmjs`.

| Property | Type | Default | Description |
| -------- | ---- | ------- | ----------- |
| `enabled` | `boolean` | `true` | Enable the docs runtime |
| `configPath` | `string` | auto-detected | Path to `docs.config.ts`, relative to the Farm application directory |
| `config` | `Partial<DocsConfig>` | — | Inline docs config overrides |
| `codeBlockThemes` | `FarmDocsCodeBlockThemes` | — | Shiki themes used to compile Markdown and MDX code blocks |
| `contentRoots` | `string[]` | — | Additional documentation roots outside the Farm application directory. Each path is resolved relative to the application root and added to the dev server's `server.fs.allow` list so the MDX plugin can compile content from shared or repository-owned locations. |

```ts title="farm.config.ts"
import { withDocs } from "@farming-labs/farmjs";

export default withDocs({}, {
  contentRoots: ["../shared-docs"],
});
```
...
### `createDocsAuthoringMcpServer(options)`

Server-only factory for the separate protected authoring MCP. It is not registered by
`createDocsMcpServer` and is never added to public framework routes.

| Option | Type | Default | Description |
| ------ | ---- | ------- | ----------- |
| `rootDir` | `string` | required | Git project root |
| `contentDir` | `string` | `entry` | Only directory authoring tools may change |
| `entry` | `string` | `"docs"` | Docs route/content fallback |
| `branchPrefix` | `string` | `"docs/"` | Required prefix for created and published branches |
| `doctorCommand` | command argv tuple | — | Shell-free command executed by `authoring_run_doctor` |
| `allowPublish` | `boolean` | `false` | Register commit/push/draft-PR publication |
| `baseBranch` | `string` | repository default | Draft PR base branch |

Use `runDocsAuthoringMcpStdio()` or `docs mcp author` for local stdio. If a team wraps the factory
in HTTP, the transport must require an authoring-specific OAuth scope. Public docs MCP remains
read-only.
...
| Property | Type | Default | Description |
| -------- | ---- | ------- | ----------- |
| `contentChanges` | `boolean \| DocsAgentContentChangesConfig` | `true` | Enable or configure the content-change feed. Set `false` to opt out. |
| `contentChanges.enabled` | `boolean` | `true` | Whether runtime adapters serve `response=changes`. |
| `contentChanges.maxSnapshots` | `number` | `8` | Maximum number of content snapshots retained for delta computation. Capped at `64`. |
| `contentChanges.loadSnapshot` | `(key: string) => Promise<DocsContentSnapshot \| null>` | — | Optional callback to load a previously saved snapshot from durable storage. |
| `contentChanges.saveSnapshot` | `(key: string, snapshot: DocsContentSnapshot) => Promise<void>` | — | Optional callback to persist a snapshot for future delta requests. |

#### Content-change feed endpoint

All five framework adapters handle `GET /api/docs?audience=agent&response=changes`. The response carries `Content-Type: application/json`, an `ETag` derived from the `indexGeneration` digest, and standard `Cache-Control` headers. Conditional `GET` with `If-None-Match` returns `304` when the index has not changed.

The response format is `docs-content-changes.v1`:

| Field | Type | Description |
| ----- | ---- | ----------- |
| `format` | `"docs-content-changes.v1"` | Fixed format identifier |
| `indexGeneration` | `string` | SHA-256 digest of the current content index |
| `mode` | `"snapshot" \| "delta" \| "reset"` | Feed mode selected for this response |
| `resetRequired` | `boolean` | `true` when the caller must discard its local state and re-index |
| `documentCount` | `number` | Total number of documents in the current index |
| `added` | `Array<{ canonicalUrl, digest, content? }>` | Documents added since the `since` generation |
| `modified` | `Array<{ canonicalUrl, digest, content? }>` | Documents whose digest changed since the `since` generation |
| `removed` | `Array<{ canonicalUrl }>` | Documents removed since the `since` generation |

Query parameters:

| Parameter | Description |
| --------- | ----------- |
| `audience` | Must be `agent` to select the content-changes response |
| `response` | Must be `changes` |
| `since` | Optional `indexGeneration` value from a previous response; omit for a full snapshot |

The agent discovery manifest gains `capabilities.contentChanges: boolean` and a top-level `contentChanges` block describing the endpoint, supported modes, query parameters, and HTTP validators (`etag: true`, `bodyFree: true`).

### `DocsAgentSkillsInput` and `DocsAgentSkillsConfig`

```ts
type DocsAgentSkillsInput =
  | string
  | readonly string[]
  | {
      ...
      progressiveDisclosure?: {
        ...
        compatibility?: "when-needed" | "always" | "off";
        checkScripts?: boolean;
      };
    };
```

Each path is relative to the project root. It may name a `SKILL.md`, its containing skill
directory, or a collection directory whose descendants contain `SKILL.md` files. A path may reach
another project inside the same workspace, but it cannot escape the workspace. Only non-symlink
regular files and directories are accepted. Published content is limited to `SKILL.md` and regular
files below `references/`, `scripts/`, and `assets/`.

The project-root `skill.md` remains the site-level compatibility skill. It is always published
alongside `agent.skills`, using a generated fallback when that root file is absent. Duplicate skill
names are rejected.

`docs doctor --agent` analyzes every configured skill. `docs review` runs the same analysis when a
configured skill, companion file, or docs config changes. Defaults follow the Agent Skills
progressive-disclosure guidance: at most 500 lines, an approximately 5,000-token instruction body,
and one local Markdown reference hop. The checks also report broken, escaping, or unpublished
references; environment-dependent skills without `compatibility`; and bundled scripts without
dependency and validation guidance. Override the thresholds with `progressiveDisclosure`.

Discovery preserves both client generations:

- `/.well-known/agent-skills/index.json` is the Agent Skills v0.2 index
- a skill containing only `SKILL.md` uses
  `/.well-known/agent-skills/<name>/SKILL.md` as its indexed `skill-md` artifact
- a skill with companions uses `/.well-known/agent-skills/<name>.tar.gz` as a deterministic indexed
  archive; the SHA-256 digest covers the exact archive bytes
- each safe file is also served at `/.well-known/agent-skills/<name>/<path>` with its own hash
- `/.well-known/skills/index.json`, `/.well-known/skills/<name>/skill.md`, and legacy companion
  paths support older clients
- Static Agent Bundles emit the same indexes, artifacts, direct files, and hashes
- MCP registers every file at `docs://skills/<name>/<path>`; text-compatible files use `text`, and
  binary files use a base64 `blob`

### `DocsAgentA2AConfig`

The generated document follows the
[A2A v1 Agent Card](https://a2a-protocol.org/v1.0.0/specification/#8-agent-discovery-the-agent-card)
wire shape.
...
| `supportedInterfaces` | `readonly DocsAgentA2AInterfaceConfig[]` | yes* | Ordered implemented interfaces; the first is preferred |
| `interfaceUrl` | `string` | deprecated alternative | Single-interface shorthand; prefer `supportedInterfaces` |
| `documentationUrl` | `string` | no | Absolute HTTPS documentation URL; HTTP is limited to loopback development |
| `provider` | `{ organization: string; url: string }` | no | Provider identity and absolute HTTPS URL outside loopback |
| `version` | `string` | no | Agent version; defaults to `"1.0.0"` |
| `iconUrl` | `string` | no | Absolute HTTPS icon URL; HTTP is limited to loopback development |
| `capabilities` | `DocsAgentA2ACapabilities` | no | Implemented streaming, push notification, extension, and extended-card capabilities |
| `defaultInputModes` | `readonly string[]` | no | Agent-wide input media types; defaults to `["text/plain"]` |
| `defaultOutputModes` | `readonly string[]` | no | Agent-wide output media types; defaults to `["text/plain"]` |
| `skills` | `readonly DocsAgentA2ASkill[]` | yes with `supportedInterfaces` | Explicit capabilities implemented by the A2A service; optional only for the deprecated shorthand |
| `securitySchemes` | `Readonly<Record<string, DocsAgentA2ASecurityScheme>>` | no | Named API key, HTTP auth, OAuth 2, OpenID Connect, or mutual TLS schemes |
| `securityRequirements` | `readonly DocsAgentA2ASecurityRequirement[]` | no | Agent-wide alternatives over named security schemes |
| `protocolVersion` | `string` | deprecated shorthand only | Protocol for `interfaceUrl`; retains the historical `"0.3"` default |
| `protocolBinding` | `string` | deprecated shorthand only | Binding for `interfaceUrl`; defaults to `"HTTP+JSON"` |

`supportedInterfaces` must contain at least one `{ url, protocolBinding?, protocolVersion?,
tenant? }` entry. Each entry defaults to A2A protocol `1.0` over `HTTP+JSON`. The core bindings are
`JSONRPC`, `GRPC`, and `HTTP+JSON`; a custom binding is an absolute URI. Ordering is meaningful: the
first interface is the client's preferred starting point. The deprecated `interfaceUrl` form
cannot be combined with `supportedInterfaces`; it retains protocol `0.3` as its default so existing
services are not silently misadvertised. Set its `protocolVersion` explicitly for other versions.
Core interfaces and all HTTP card/authentication metadata require HTTPS outside loopback
development. Custom bindings use a secure binding-appropriate absolute URL such as WSS or MQTTS.

`DocsAgentA2ASkill` requires `id`, `name`, `description`, and non-empty `tags`; it can also set
`examples`, capability-specific `inputModes` and `outputModes`, and `securityRequirements`. It
does not have a URL in A2A v1. URLs for configured Agent Skill documents and artifacts remain in
`/.well-known/agent-skills/index.json`, its legacy compatibility index, and MCP resources. The
preferred `supportedInterfaces` form requires explicit A2A skills. Published documentation skills
are projected only when `a2a.skills` is omitted from the deprecated shorthand, preserving its
previous behavior.

Security schemes use the A2A v1 discriminated wrappers, and each requirement maps a configured
scheme name to its scope list. For example:
...
securityRequirements: [
  {
    schemes: {
      bearer: { list: ["docs.read"] },
    },
  },
],
```

Use exactly one wrapper per named scheme. Requirement objects are alternatives; multiple schemes
inside one object are required together. `capabilities.extendedAgentCard: true` requires at least
one declared security scheme and a non-empty applicable security requirement.

When `a2a` is set, `/.well-known/agent-card.json` uses the strict v1 shape with
`supportedInterfaces`; it does not emit legacy top-level `url`, `protocolVersion`,
`preferredTransport`, or per-skill `url` fields. Capabilities default to non-streaming with no push
notifications, while security metadata is omitted unless configured. When `a2a` is omitted, the
route is not published. Do not point an advertised interface at a docs, Ask AI, or MCP route; those
transports do not implement A2A.

### `DocsAgentEvaluationsConfig`

| Property | Type | Default | Description |
| -------- | ---- | ------- | ----------- |
| `enabled` | `boolean` | `true` | Enable configured golden-task evaluation |
| `tokenBudget` | `number` | `4000` | Default hard UTF-8 context-byte ceiling |
...
Each task requires `id`, `query`, and `expect.relevantSources`. It can set `filters.framework`,
`filters.version`, `filters.locale`, per-task `surface`, and per-task `tokenBudget` or `topK`.
Expectations can include `allowedSources`, `forbiddenSources`, `requiredCitations`, `minRecallAtK`,
`maxFirstRelevantRank`, `minUsefulByteRatio`, `scope`, `answer`, and code-fence matches through
`examples`. `filters` narrow retrieval; `expect.scope` asserts the framework, version, or locale of
the returned sources without narrowing them first.

`DocsAgentEvaluationAnswerProvider` is either `{ provider: "callback", run }` or
`{ provider: "http", endpoint, headers?, timeoutMs? }`. The callback receives the task id, query,
filters, surface, context, retrieved sources, and an abort signal, but never receives the task
expectations. The serializable HTTP request omits that callback-only signal. Both
providers return `{ text, citations? }`. HTTP auth is opt-in through `headers`, and header values
never appear in reports. Managed external search and HTTP requests require `allowNetwork: true`,
while callbacks remain explicit user code.

#### `DocsAgentGoldenSafetyExpectation`

Optional adversarial safety assertions on a golden task. All fields are evaluator-only and must never contain real credentials or private data. Exported as `DocsAgentGoldenSafetyExpectation` from `@farming-labs/docs`.

| Property | Type | Description |
| -------- | ---- | ----------- |
| `promptInjection` | `DocsAgentGoldenPromptInjectionExpectation` | Assert prompt-injection resistance using canary markers |
| `poisonedCitations` | `boolean` | Assert that retrieval rejects injected citation payloads |
| `authenticatedContent` | `DocsAgentGoldenAuthenticatedContentExpectation` | Assert that authenticated content does not leak into public retrieval |
| `freshness` | `DocsAgentGoldenFreshnessExpectation` | Assert retrieval-index generation and per-source content digests |
| `rejectConflictingFrameworkVersions` | `boolean` | Assert that sources with conflicting framework version provenance are rejected |
| `deletedSectionTombstones` | `boolean` | Assert that deleted-section tombstones are surfaced |
| `queryVariants` | `DocsAgentGoldenQueryVariant[]` | Additional query variant kinds to evaluate alongside the primary query |

`DocsAgentGoldenPromptInjectionExpectation` has `markers: string[]` (canary fragments that must appear in retrieved context) and an optional `forbiddenAnswerText?: string[]` (fragments the answer must not repeat; defaults to `markers`).

`DocsAgentGoldenAuthenticatedContentExpectation` has optional `forbiddenSources?: string[]` (protected URLs that public retrieval must not expose) and `forbiddenText?: string[]` (canary fragments that must not appear in context or answers).

`DocsAgentGoldenFreshnessExpectation` has optional `indexGeneration?: string` (expected retrieval-index generation digest) and `sourceDigests?: Record<string, string>` (expected full-document digest keyed by canonical page or section URL).

`DocsAgentGoldenQueryVariant` has `kind: DocsAgentGoldenQueryVariantKind` (a string such as `"rephrased"` or `"adversarial"`).

All five types are exported from `@farming-labs/docs` and `@farming-labs/docs/server`.

`DocsAgentGoldenExpectedExample.verification` accepts `"present"`, `"syntax"`, or `"execute"`.
Runnable examples default to syntax verification and explicitly non-runnable examples default to
presence. Execution is never inferred: it requires `allowNetwork: true`,
`verification: "execute"`, and an enabled `codeBlocks.validate` configuration in `"report"` mode;
only an actual `PASS` receives credit.

Evaluation is offline by default on `mcp-context`, with no implicit model or network call. The
`configured-search` and `ask-ai-context` surfaces exercise the real configured retrieval/context
paths, and provider failures or timeouts fail instead of falling back to a different surface.
Configured-search preserves provider ranking and does not supplement it with local hits. See
[Golden agent evaluations](/docs/configuration#golden-agent-evaluations) for a complete example.

---

## `DocsReviewConfig`

Docs Review configuration for changed docs files, score thresholds, and GitHub Actions behavior.

Omitted `review` config is enabled by default. Set `review: false` to opt out of the review command
and generated workflow. The generated CI file is a small repository-root wrapper around the hosted
reusable workflow at `farming-labs/docs/.github/workflows/docs-review-reusable.yml@main`.

| Property | Type | Default | Description |
| -------- | ---- | ------- | ----------- |
| `enabled` | `boolean` | `true` | Enable Docs Review |
| `ci` | `boolean \| DocsReviewCiConfig` | enabled | GitHub Actions behavior and workflow generation |
| `score` | `DocsReviewScoreConfig` | `{ threshold: 80 }` | Healthy score threshold and severity weights |
| `rules` | `DocsReviewRulesConfig` | default severities | Per-rule severity overrides |
...
| -------- | ------- | ----------- |
| `brokenLinks` | `"error"` | Check internal docs links |
| `frontmatter` | `"error"` | Check title and description frontmatter |
| `duplicateSlugs` | `"error"` | Check duplicate docs routes |
| `invalidMdx` | `"error"` | Check markdown/MDX read and frontmatter parsing |
| `configExamples` | `"warn"` | Check config examples when possible |
| `codeFenceMetadata` | `"warn"` | Check code fences for metadata such as `title` |
| `runnableMetadata` | `"warn"` | Check runnable snippets for package manager metadata |
| `agentContext` | `"warn"` | Validate structured contracts and detect duplicate, generic, incomplete, or ambiguous agent context |
| `commandHealth` | `"warn"` | Statically validate package managers, scripts, working directories, and docs CLI commands |
| `relatedCoverage` | `"suggestion"` | Check valid related-page coverage for actionable docs |
| `configConfidence` | `"warn"` | Report partial static fallback when the config module cannot be evaluated |
| `agentSurfaceDrift` | `"error"` | Detect discovery, resolved config, contract, route, MCP tool, and schema drift |
| `goldenTasks` | `"warn"` | Report failed or unmeasured configured golden tasks |
| `agentSkills` | `"warn"` | Check configured skills for progressive disclosure, reference integrity, compatibility, and script guidance |

Rule severities accept `"off"`, `"suggestion"`, `"warn"`, or `"error"`.
...
See [CLI](/docs/cli) for `docs robots generate` and [Configuration](/docs/configuration) for the
...
### `SearchSyncOptions` — additional sync fields

These options are available on the `search sync` CLI command and on the `SearchSyncOptions` interface:

| Property | Type | Description |
| -------- | ---- | ----------- |
| `syncNamespace` | `string` | Optional namespace that scopes the synced corpus. Maximum 1024 characters. Set via `--sync-namespace` flag or the `DOCS_SEARCH_SYNC_NAMESPACE` environment variable. A value exceeding 1024 chars is rejected with error code `invalid-search-sync-namespace`. |
| `siteUrl` | `string` | Canonical public URL embedded in indexed documents during sync. Set via `--site-url` flag. |

### `DocsSearchSourcePage` — additional fields

| Property | Type | Description |
| -------- | ---- | ----------- |
| `canonicalUrl` | `string` | Optional canonical public URL override for this page. Relative values are resolved against the request base URL. |
| `agentLastModified` | `string` | ISO-8601 modification time for an explicit agent-source override file (e.g. `agent.md`), tracked separately from the page's own `lastModified`. |

### Retrieval provenance types and exports

The following are exported from `@farming-labs/docs` and `@farming-labs/docs/server`:

| Export | Kind | Description |
| ------ | ---- | ----------- |
| `buildDocsRetrievalDigestProjection` | function | Builds a projection object containing a SHA-256 digest (`sha256:<hex>`) of normalized page content, suitable for change-detection in search indexes. |
| `enrichDocsSearchDocumentsWithProvenance` | function | Attaches retrieval provenance metadata (scope, canonical URL, digest) to an array of search documents. |
| `resolveDocsRetrievalLastModified` | function | Resolves the canonical last-modified date for a page, preferring the `lastmod` frontmatter value over the filesystem mtime. |
| `DocsRetrievalSourceProvenance` | type | Normalized provenance metadata attached to a retrieved source, including `canonicalUrl`, `lastModified`, `digest`, and scope fields. |
| `DocsRetrievalSourceScope` | type | Applicability metadata for a retrieved source: `audience`, `locale`, `framework`, `version`, `versionGroups`, and `package`. |
| `EnrichDocsSearchDocumentsWithProvenanceOptions` | type | Options accepted by `enrichDocsSearchDocumentsWithProvenance`. |

### HTTP cache integrity helpers

The following are exported from `@farming-labs/docs`:

| Export | Kind | Description |
| ------ | ---- | ----------- |
| `createDocsCacheableResponse` | function | Build a cacheable `Response` for an agent-surface request. Computes a strong `ETag` (`"<64-hex-char sha256>"`), an RFC 9530 `Content-Digest: sha-256=:…:` header, and an optional `Last-Modified` header. Returns an empty-body response for `HEAD` requests and `304 Not Modified` when `If-None-Match` matches. |
| `formatDocsContentDigest` | function | Format a SHA-256 hex digest as an RFC 9530 `Content-Digest` field value (`sha-256=:…:`). |
| `requestHasFreshDocsDate` | function | Return `true` when the request's `If-Modified-Since` header indicates the client already has a fresh copy. |
| `requestMatchesDocsEtag` | function | Return `true` when the request's `If-None-Match` header matches the supplied strong ETag. |
| `resolveDocsHttpDate` | function | Parse a `string`, `Date`, or `null` into an RFC 7231 HTTP-date string, returning `null` for invalid or absent values. |
| `sha256DocsContent` | function | Hash exact bytes (string or `Uint8Array`) with portable SHA-256; returns the lowercase hex digest. Works in Node, edge, and browser runtimes. |
| `digestDocsContent` | function | Hash a string or `Uint8Array` and return the result in `sha256:<hex>` provenance format. |
| `DocsCacheableResponseOptions` | type | Options accepted by `createDocsCacheableResponse`: `request`, `content`, `status?`, `headers?`, `sha256?` (pre-computed hex digest), and `lastModified?` (`string \| Date \| null`). |

**ETag format change:** Agent-surface responses now use a **strong** ETag (`"<64-hex-char sha256>"`) instead of the previous weak form (`W/"…"`). Clients that cache or compare ETags must update their `If-None-Match` logic accordingly. Conditional `GET` and `HEAD` requests with a matching strong ETag receive `304 Not Modified` with the `Content-Digest` header preserved.

**`lastModified` on discovery responses:** `DocsStandardsDiscoveryResponseOptions` and `CreateDocsStandardsResponseOptions` both accept a new optional `lastModified?: string | Date | null` field. When supplied, the response includes a `Last-Modified` header and supports `If-Modified-Since` conditional requests.
...
| Property | Type | Description |
| -------- | ---- | ----------- |
| `value` | `string` | Normalized filter value |
| `count` | `number` | Number of source pages that carry this value |
...
| Property | Type | Description |
| -------- | ---- | ----------- |
| `valueCount` | `number` | Number of distinct values before the response limit is applied |
| `truncated` | `boolean` | Whether additional values were omitted from this response |
| `values` | `DocsSearchFacetValue[]` | Ordered list of values and counts |
| `total` | `number` | Total number of distinct values. Alias of `valueCount` for pagination clients |
| `hasMore` | `boolean` | Whether another page of values can be requested using `nextCursor` |
| `nextCursor` | `string \| undefined` | Opaque cursor for the next page. Omitted when this is the final page |

### `DocsSearchFacetsResponse`

Cheap, body-free discovery response for the search scopes available in an index. Returned by `GET /api/docs?audience=agent&response=facets` with `Cache-Control: public, max-age=60`.

Two optional query parameters refine the `?response=facets` request:

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `facet` | `framework \| version \| package \| tags` | Pin the response to a single field and enable cursor-based pagination for that field. Requires `response=facets`. |
| `limit` | `integer 1–100` | Maximum number of facet values to return per page. Defaults to the built-in cap when omitted. |
| `cursor` | `string` | Opaque continuation token returned as `nextCursor` in a previous response. Accepted alongside `response=facets` (and the existing `response=structured`) when `facet` is set. |

When `facet` is present, only the named field is computed and returned. Check `hasMore` in the response to determine whether another page is available, then pass the returned `nextCursor` value as `cursor` in the next request to retrieve it.
| Property | Type | Description |
| -------- | ---- | ----------- |
| `format` | `"docs-search-facets.v1"` | Fixed format identifier |
| `filters` | `DocsSearchFilters` | Active filters applied to the facet computation |
| `matchedPageCount` | `number` | Number of source pages matched after filters |
| `facets` | `Record<string, DocsSearchFacet>` | Facet values keyed by field name (`framework`, `version`, `package`, `tags`) |

### `BuildDocsSearchFacetsOptions`

Options accepted by `buildDocsSearchFacets`. Exported from `@farming-labs/docs`.

| Property | Type | Description |
| -------- | ---- | ----------- |
| `pages` | `DocsSearchSourcePage[]` | Source pages to compute facets over |
| `search` | `DocsSearchConfig` | Active search config |
| `audience` | `"human" \| "agent"` | Content projection used when filtering pages |
| `locale` | `string` | Active locale |
| `baseUrl` | `string` | Public base URL for provenance |
| `filters` | `DocsSearchFilters` | Optional filters to apply before computing facets |
| `indexGeneration` | `string` | Digest of the current index for cache freshness |
| `scope` | `object` | Optional audience/locale scope |
| `facet` | `DocsSearchFilterField` | Optional single field to compute and paginate (`framework`, `version`, `package`, or `tags`) |
| `limit` | `number` | Maximum number of facet values to return (1–100) |
...
Builds a `DocsSearchFacetsResponse` from source pages without fetching page content. Exported from `@farming-labs/docs`.

```ts
import { buildDocsSearchFacets } from "@farming-labs/docs";

const response = await buildDocsSearchFacets(options);
// response.format === "docs-search-facets.v1"
```
...
<Callout type="info" title="Renderer dependencies are bundled">
  You do not need to install `fumadocs-openapi`, `fumadocs-ui`, `@scalar/core`, or related packages separately. The `"fumadocs"` renderer is bundled by `@farming-labs/next`; the `"scalar"` renderer is bundled by `@farming-labs/docs` and the other framework adapters.
</Callout>
...
| `renderer`  | `"fumadocs" \| "scalar"` | framework-aware | Which renderer to use: `"fumadocs"` is bundled by `@farming-labs/next`; `"scalar"` is bundled by `@farming-labs/docs` and the other adapters. Defaults to `"fumadocs"` for Next.js and `"scalar"` for other frameworks |
...
| `requireComment` | `boolean`                              | `false`           | Require a non-empty comment before feedback can be submitted |
...
- agents can discover APIs through `GET` or `HEAD /.well-known/api-catalog` (RFC 9727), and can
  discover integrity-checked skills through `GET` or `HEAD /.well-known/agent-skills/index.json`
- agents should discover site identity, locale config, capability flags, search, active markdown routes, `Accept: text/markdown`, `Signature-Agent` support, JSON-LD structured data, `llms.txt`, OpenAPI schema routes, sitemap routes, `robots.txt`, `AGENTS.md`, `skill.md`, skills, MCP, and feedback routes with `GET /.well-known/agent.json`; `GET /.well-known/agent` is the public fallback and `GET /api/docs/agent/spec` is the canonical framework route
...
`analyzeDocsAgentFeedback()` converts recurring failed reports into evidence clusters, golden-task
drafts, issue drafts, and a draft docs PR description. `readDocsAgentFeedbackFile()` accepts JSON
arrays/objects and JSON Lines. Both are exported from `@farming-labs/docs/server`; the CLI wrapper
is `docs agent feedback --input <path> [--write]`.
...
curl -I "http://127.0.0.1:3000/.well-known/api-catalog"

curl "http://127.0.0.1:3000/.well-known/agent-skills/index.json"
...
| Property      | Type      | Default       | Description                                        |
| ------------- | --------- | ------------- | -------------------------------------------------- |
| `format`      | `"markdown" \| "text"` | `"markdown"` | Content format copied by the button: `"markdown"` fetches the `.md` route; `"text"` reads visible page text |
| `enabled`     | `boolean` | `false`       | Show the button                                    |
| `label`       | `string`  | `"Copy page"` | Button label shown before the page is copied       |
| `copiedLabel` | `string`  | `"Copied!"`   | Button label shown after a successful copy         |
| `includeTitle` | `boolean` | `false`      | Prepend the current page title to copied content. In `"markdown"` format this produces a `# Title` heading; in `"text"` format it prepends the plain title. Skipped if the content already begins with the title. |
...
| `target`    | `"markdown" \| "page" \| "source" \| "github"` | `"markdown"` | URL inserted into `{url}` for built-in provider prompts |
| `prompt`    | `string`             | `"Read this documentation: {url}"` | Prompt text for built-in providers                       |
...
| `prompt`            | `string`    | Per-provider prompt override                                                      |
| `mode`              | `"web" \| "app"` | Cursor-only option. `"app"` uses the Cursor app deeplink                    |
| `urlTemplate`       | `string`    | Custom URL template. Supports `{prompt}`, `{url}`, `{pageUrl}`, `{markdownUrl}`, `{sourceUrl}`, `{mdxUrl}`, and `{githubUrl}`. |
| `promptUrlTemplate` | `string`    | Optional URL template used by the built-in `Prompt` component. `{prompt}` is replaced with the prompt text. |
...
Controls the "Last updated" date shown on docs pages.

| Property   | Type                        | Default          | Description                                                     |
| ---------- | --------------------------- | ---------------- | --------------------------------------------------------------- |
| `enabled`  | `boolean`                   | `true`           | Show or hide the last-updated date                              |
| `label`    | `string`                    | `"Last updated"` | Text prefix shown before the formatted date                     |
| `position` | `"footer" \| "below-title"` | `"footer"`       | Where to render the date: next to "Edit on GitHub" or below the page title |
...
Set `label` to an empty string to show only the formatted date with no prefix. Use any locale-appropriate string to rebrand the prefix text (e.g. `"Dernière mise à jour"` for French docs).
...
| `format`         | `"long" \| "short"` | `"long"` | Label style: `3 min read` for `"long"` or `3 min` for `"short"` |
| `includeCode`    | `boolean`           | `false`  | Whether fenced and inline code blocks count toward the reading time estimate. Disabled by default so the label reflects human prose. Set to `true` for code-heavy guides. |
...
- set `includeCode: true` to preserve fenced and inline code word counts in the estimate (useful for code-heavy guides where examples make up a significant portion of the page)
- `format` changes only the rendered label; it does not affect word counting or per-page overrides
...
The agent discovery spec advertises this as `capabilities.structuredData` and
...
| `agent`       | `PageAgentFrontmatter` | Per-page structured task contract and compaction token budget |
...
- MCP list-page results contain only `hasContract`, `task`, `outcome`, and `appliesTo`; call
  `list_tasks` or `read_task` when an agent needs actionable discovery or the full contract
- `docs review` reports malformed or unknown fields with closest-name suggestions, missing `task` or `outcome`, commands without
  verification, and side effects without rollback guidance
...
| --------- | ------------------------------ | -------------------------- |
| `string`  | `title: "Hello"` or `title: Hello` | `title`, `description`, `icon`, `ogImage`, `framework`, `version`, `locale` |
| `number`  | `order: 1`                     | `order`                    |
| `boolean` | `hidden: true`                 | Optional (e.g. hide from sidebar) |
| `string[]`| `tags: [a, b]` or `tags: ["a", "b"]` | `tags`                  |

You can use other YAML types (e.g. nested objects) for custom keys; the fields in the table above are used by the framework for sidebar, SEO, JSON-LD structured data, OG, search, scoped MCP context, and machine-readable markdown. Quote numeric `version` values so they remain strings for MCP filtering.

### How frontmatter is passed

- **Next.js** — The MDX pipeline uses `remark-frontmatter` and `remark-mdx-frontmatter` with `name: "metadata"`. The parsed frontmatter is exposed as the **`metadata`** export from the page. The layout and `generateMetadata` receive it when resolving the page (e.g. via `getPage(params)`), and it is used for `createPageMetadata`, sidebar tree (title, icon, order), description maps, and JSON-LD structured data.
- **TanStack Start / SvelteKit / Astro / Nuxt** — The docs loader (e.g. `import.meta.glob` or content layer) reads each `.md`/`.mdx` file, parses frontmatter, and builds the nav tree and page data. Frontmatter is passed to the layout as **page data** (e.g. `page.data` or the props your content component receives). The same fields (`title`, `description`, `icon`, `order`, `tags`, `ogImage`, `framework`, `version`, `locale`) are used for sidebar, meta tags, JSON-LD structured data, search, and scoped MCP context.

So in all frameworks, the **same frontmatter fields** drive titles, descriptions, icons, ordering, JSON-LD, and OG; only the mechanism (MDX `metadata` vs. content loader `page.data`) differs. The `related` field is intentionally machine-readable first: add URL strings, and `.md` routes plus MCP page output render them next to `Description` as `Related: /docs/a, /docs/b` without adding a visible related-links block to the human page UI. A sibling `agent.md` remains the page-content override, so add a `Related:` line there manually if the override should include it.
...
| Import                                       | Description              |
| -------------------------------------------- | ------------------------ |
| `@farming-labs/theme/default/css`            | Default theme            |
...
### `withDocs(nextConfig, docsConfig?)`
...
The optional second argument supplies the live docs config when build-time routing cannot be read
statically from an imported, spread, or shorthand `docs.config` value. Protected MCP users should
pass it so RFC 9728 rewrites and the incompatible-`basePath` check use the resolved configuration.
