...
| `agent`       | `DocsAgentConfig`                               | —                | Compaction defaults and deterministic agent usefulness evaluations                   |
...
| `evaluations` | `boolean \| DocsAgentEvaluationsConfig` | Deterministic golden tasks used by `docs doctor` and `docs review` |
...
`filters.version`, `filters.locale`, and per-task `tokenBudget` or `topK`. Expectations can include
`allowedSources`, `forbiddenSources`, `requiredCitations`, `minRecallAtK`,
`maxFirstRelevantRank`, `minUsefulByteRatio`, and runnable code-fence matches through `examples`.

The evaluator is offline and deterministic. It does not execute commands; it measures retrieval,
canonical citation integrity, framework/version selection, executable example metadata, and exact
UTF-8 context usage. See [Golden agent evaluations](/docs/configuration#golden-agent-evaluations)
...
| `goldenTasks` | `"warn"` | Report failed or unmeasured deterministic golden tasks |
