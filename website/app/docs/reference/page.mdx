...
| `observability` | `boolean \| DocsObservabilityConfig`          | `false`          | Event stream for logs, traces, metrics, callbacks, Ask AI, MCP, and agent routes     |
...
## `observability`, `analytics`, and `DocsAnalyticsConfig`

Built-in event stream for product analytics and operational observability. Prefer `observability`
for logs, traces, metrics, and agent debugging. `analytics` uses the same config shape and remains
supported for existing projects. See [Analytics](/docs/customization/analytics) for the full guide
and event list.
